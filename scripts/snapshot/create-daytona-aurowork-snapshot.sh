...
SNAPSHOT_NAME="${1:-${DAYTONA_SNAPSHOT_NAME:-aurowork-runtime}}"
...
LOCAL_IMAGE_TAG="${DAYTONA_LOCAL_IMAGE_TAG:-aurowork-daytona-snapshot:${SNAPSHOT_NAME//[^a-zA-Z0-9_.-]/-}}"

AUROWORK_ORCHESTRATOR_VERSION="${AUROWORK_ORCHESTRATOR_VERSION:-$(node -e 'const fs=require("fs"); const pkg=JSON.parse(fs.readFileSync(process.argv[1], "utf8")); process.stdout.write(String(pkg.version));' "$ROOT_DIR/apps/orchestrator/package.json")}"
...
echo "- aurowork-orchestrator@$AUROWORK_ORCHESTRATOR_VERSION" >&2
...
docker buildx build \
  ...
  -t "$LOCAL_IMAGE_TAG" \
  -f "$DOCKERFILE" \
  --build-arg "AUROWORK_ORCHESTRATOR_VERSION=$AUROWORK_ORCHESTRATOR_VERSION" \
