...
const autoBuildEnabled =
  process.env.AUROWORK_DEV_HEADLESS_WEB_AUTOBUILD == null
    ? true
    : readBool(process.env.AUROWORK_DEV_HEADLESS_WEB_AUTOBUILD);
...
const remoteAccessEnabled = readBool(process.env.AUROWORK_REMOTE_ACCESS);
const host = remoteAccessEnabled ? "0.0.0.0" : "127.0.0.1";
const viteHost = process.env.VITE_HOST ?? process.env.HOST ?? host;
const publicHost = process.env.AUROWORK_PUBLIC_HOST ?? null;
const clientHost = publicHost ?? (host === "0.0.0.0" ? "127.0.0.1" : host);
const workspace = process.env.AUROWORK_WORKSPACE ?? cwd;
const auroworkPort = await resolvePort(process.env.AUROWORK_PORT, "127.0.0.1");
const webPort = await resolvePort(process.env.AUROWORK_WEB_PORT, "127.0.0.1");
const auroworkToken = process.env.AUROWORK_TOKEN ?? randomUUID();
const auroworkHostToken = process.env.AUROWORK_HOST_TOKEN ?? randomUUID();
const auroworkServerBin = path.join(
  cwd,
  "apps/server/dist/bin/aurowork-server",
);
...
const ensureAuroworkServer = async () => {
  try {
    await access(auroworkServerBin);
  } catch {
    if (!autoBuildEnabled) {
      logLine(
        `[dev:headless-web] Missing AuroWork server binary at ${auroworkServerBin}`,
      );
      logLine(
        "[dev:headless-web] Auto-build disabled (AUROWORK_DEV_HEADLESS_WEB_AUTOBUILD=0)",
      );
      logLine(
        "[dev:headless-web] Run: pnpm --filter aurowork-server build:bin",
      );
      logLine(
        "[dev:headless-web] Or unset/enable AUROWORK_DEV_HEADLESS_WEB_AUTOBUILD to auto-build.",
      );
      process.exit(1);
    }
    ...
    logLine(
      "[dev:headless-web] Auto-building: pnpm --filter aurowork-server build:bin",
    );
    try {
      await runCommand("pnpm", ["--filter", "aurowork-server", "build:bin"]);
      await access(auroworkServerBin);
    } catch (error) {
      logLine(
...
const auroworkUrl = `http://${clientHost}:${auroworkPort}`;
const webUrl = `http://${clientHost}:${webPort}`;
// In practice we want opencode-router on for end-to-end messaging tests.
// Allow opt-out via AUROWORK_DEV_OPENCODE_ROUTER=0.
const opencodeRouterEnabled =
  process.env.AUROWORK_DEV_OPENCODE_ROUTER == null
    ? true
    : readBool(process.env.AUROWORK_DEV_OPENCODE_ROUTER);
const opencodeRouterRequired = readBool(
  process.env.AUROWORK_DEV_OPENCODE_ROUTER_REQUIRED,
);
const viteEnv = {
  ...process.env,
  HOST: viteHost,
  PORT: String(webPort),
  VITE_AUROWORK_URL: process.env.VITE_AUROWORK_URL ?? auroworkUrl,
  VITE_AUROWORK_PORT: process.env.VITE_AUROWORK_PORT ?? String(auroworkPort),
  VITE_AUROWORK_TOKEN: process.env.VITE_AUROWORK_TOKEN ?? auroworkToken,
};
const headlessEnv = {
  ...process.env,
  AUROWORK_WORKSPACE: workspace,
  AUROWORK_HOST: host,
  AUROWORK_REMOTE_ACCESS: remoteAccessEnabled ? "1" : "0",
  AUROWORK_PORT: String(auroworkPort),
  AUROWORK_TOKEN: auroworkToken,
  AUROWORK_HOST_TOKEN: auroworkHostToken,
  AUROWORK_SERVER_BIN: auroworkServerBin,
  AUROWORK_SIDECAR_SOURCE: process.env.AUROWORK_SIDECAR_SOURCE ?? "external",
  OPENCODE_ROUTER_BIN: process.env.OPENCODE_ROUTER_BIN ?? opencodeRouterBin,
};

await ensureAuroworkServer();
...
logLine(`[dev:headless-web] AuroWork server: ${auroworkUrl}`);
...
logLine(
  `[dev:headless-web] OpenCodeRouter: ${opencodeRouterEnabled ? "on" : "off"} (set AUROWORK_DEV_OPENCODE_ROUTER=0 to disable)`,
);
logLine(`[dev:headless-web] AUROWORK_TOKEN: ${auroworkToken}`);
logLine(`[dev:headless-web] AUROWORK_HOST_TOKEN: ${auroworkHostToken}`);
logLine(
