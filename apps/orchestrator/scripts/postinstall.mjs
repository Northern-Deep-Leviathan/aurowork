...
function binaryName() {
  ...
  return platform === "windows" ? "aurowork.exe" : "aurowork"
}

function fallbackAssetName() {
  const { platform, arch } = detect()
  return `aurowork-bun-${platform}-${arch}${platform === "windows" ? ".exe" : ""}`
}
...
function readOwnVersion() {
  ...
  if (!version) {
    throw new Error("aurowork-orchestrator: package version is missing")
  }
  return version
}

function resolveFallbackBaseUrl(version) {
  const override = String(process.env.AUROWORK_ORCHESTRATOR_DOWNLOAD_BASE_URL || "").trim()
  if (override) {
...
async function downloadFallbackBinary() {
  ...
  console.log(`aurowork-orchestrator: downloading fallback binary ${asset}`)
  ...
  if (binaryName() !== "aurowork.exe") {
    chmodSync(destination, 0o755)
  }

  console.log(`aurowork-orchestrator: installed fallback binary at ${destination}`)
}
...
async function main() {
  try {
    ...
    console.log(`aurowork-orchestrator: verified platform package: ${pkg}`)
    return
  } catch {
    if (existsSync(fallbackBinaryPath())) {
      console.log(`aurowork-orchestrator: using existing fallback binary at ${fallbackBinaryPath()}`)
      return
    }
  }
  ...
  try {
    ...
    console.error(
      `aurowork-orchestrator: failed to locate platform binary package (${pkg}).\n` +
