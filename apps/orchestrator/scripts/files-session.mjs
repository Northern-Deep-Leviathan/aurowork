...
async function runCli(args) {
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `aurowork CLI failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "aurowork-file-session-"));
...
const auroworkUrl = `http://127.0.0.1:${port}`;
...
try {
  await waitFor(`${auroworkUrl}/health`);

  const workspaces = await fetchJson(`${auroworkUrl}/workspaces`, {
  ...
  assert.equal(diskContent, "updated by aurowork cli\n");
  ...
  console.log(JSON.stringify({ ok: true, auroworkUrl, workspaceId, sessionId }, null, 2));
} catch (error) {
  console.error(
    JSON.stringify(
      {
