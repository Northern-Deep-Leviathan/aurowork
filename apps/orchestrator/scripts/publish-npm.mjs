...
if (!version) {
  throw new Error("aurowork-orchestrator version missing in apps/orchestrator/package.json")
}
...
function platformPkgName(target) {
  ...
  return `aurowork-orchestrator-${platform}-${arch}`
}
...
for (const target of targets) {
  ...
  const src = join(root, "dist", "bin", `aurowork-${target.bun}${ext}`)
  if (!existsSync(src)) {
    throw new Error(`Missing aurowork binary at ${src}. Run: pnpm --filter aurowork-orchestrator build:bin:all`)
  }
  ...
  const dest = join(bindir, `aurowork${ext}`)
  ...
  writeJson(join(dir, "package.json"), {
    name,
    version,
    description: "Platform binary for aurowork-orchestrator",
    license: "MIT",
    os: [target.os],
    cpu: [target.cpu],
    bin: {
      aurowork: `./bin/aurowork${ext}`,
      "aurowork-orchestrator": `./bin/aurowork${ext}`,
    },
    files: ["bin"],
  })
...
const meta = join(outroot, "aurowork-orchestrator")
mkdirSync(join(meta, "bin"), { recursive: true })

const wrapperSrc = join(root, "bin", "aurowork")
if (!existsSync(wrapperSrc)) {
  throw new Error(`Missing wrapper at ${wrapperSrc}`)
}
copyFileSync(wrapperSrc, join(meta, "bin", "aurowork"))
chmodSync(join(meta, "bin", "aurowork"), 0o755)
...
writeJson(join(meta, "package.json"), {
  ...
  bin: {
    ...
    "aurowork-orchestrator": "./bin/aurowork",
  },
  scripts: {
...
published.push({ name: "aurowork-orchestrator", dir: meta })
