...
if (!orchestratorVersion) {
  throw new Error(
    "aurowork-orchestrator version missing in apps/orchestrator/package.json",
  );
}
...
if (!serverVersion) {
  throw new Error(
    "aurowork-server version missing in apps/server/package.json",
  );
}
...
run("pnpm", ["--filter", "aurowork-server", "build:bin:all"], repoRoot);
...
const entries = {
  "aurowork-server": { version: serverVersion, targets: {} },
  "opencode-router": { version: routerVersion, targets: {} },
};

for (const target of targets) {
  const ext = target.id.startsWith("windows") ? ".exe" : "";
  const serverSrc = join(serverDir, `aurowork-server-${target.bun}${ext}`);
  if (!existsSync(serverSrc)) {
    throw new Error(`Missing aurowork-server binary at ${serverSrc}`);
  }
  const serverDest = join(outdir, `aurowork-server-${target.id}${ext}`);
  ...
  entries["aurowork-server"].targets[target.id] = {
...
writeFileSync(
  join(outdir, "aurowork-orchestrator-sidecars.json"),
