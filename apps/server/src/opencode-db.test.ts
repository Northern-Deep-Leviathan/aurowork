...
describe("resolveOpencodeDbPath", () => {
  ...
  test("finds orchestrator-managed OpenCode dbs under AUROWORK_DATA_DIR", async () => {
    const root = await mkdtemp(join(tmpdir(), "aurowork-orchestrator-data-"));
    const dir = join(root, "aurowork-dev-data", "xdg", "data", "opencode");
    const file = join(dir, "opencode.db");
    await mkdir(dir, { recursive: true });
    await writeFile(file, "", "utf8");

    const previousDataDir = process.env.AUROWORK_DATA_DIR;
    const previousXdg = process.env.XDG_DATA_HOME;
    const previousChannel = process.env.OPENCODE_CHANNEL;
    const previousDb = process.env.OPENCODE_DB;
    try {
      process.env.AUROWORK_DATA_DIR = root;
  ...
  test("finds legacy orchestrator-managed OpenCode dbs under AUROWORK_DATA_DIR", async () => {
    const root = await mkdtemp(join(tmpdir(), "aurowork-orchestrator-data-"));
