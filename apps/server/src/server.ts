...
export function createServerLogger(config: ServerConfig): ServerLogger {
  const runId = process.env.AUROWORK_RUN_ID ?? shortId();
    ...
    fetch: async (request: Request) => {
      ...
      try {
        ...
        if (!(error instanceof ApiError)) {
          console.error("[aurowork-server] Unhandled error:", error);
        }
        const apiError = error instanceof ApiError
          ? error
          : new ApiError(500, "internal_error", "Unexpected server error");
...
async function proxyOpencodeRequest(input: {
  ...
  headers.delete("x-aurowork-client-id");
  headers.delete("host");
...
function withCors(response: Response, request: Request, config: ServerConfig) {
  ...
  headers.set(
    ...
    "Authorization, Content-Type, X-AuroWork-Host-Token, X-AuroWork-Client-Id, X-OpenCode-Directory, X-Opencode-Directory, x-opencode-directory",
  );
...
async function requireClient(request: Request, config: ServerConfig, tokens: TokenService): Promise<Actor> {
  ...
  const clientId = request.headers.get("x-aurowork-client-id") ?? undefined;
...
async function requireHost(request: Request, config: ServerConfig, tokens: TokenService): Promise<Actor> {
  const hostToken = request.headers.get("x-aurowork-host-token");
...
function buildCapabilities(config: ServerConfig): Capabilities {
  ...
  return {
    ...
    skills: { read: true, write: writeEnabled, source: "aurowork" },
    ...
    toolProviders: {
      ...
      files: {
        ...
        inboxPath: ".opencode/aurowork/inbox/",
        outboxPath: ".opencode/aurowork/outbox/",
        maxBytes,
      },
    },
  };
...
function resolveSandboxBackend(): Capabilities["sandbox"]["backend"] {
  const raw = (process.env.AUROWORK_SANDBOX_BACKEND ?? "").trim().toLowerCase();
...
function resolveSandboxEnabled(backend: Capabilities["sandbox"]["backend"]): boolean {
  const raw = (process.env.AUROWORK_SANDBOX_ENABLED ?? "").trim().toLowerCase();
...
function resolveInboxEnabled(): boolean {
  const raw = (process.env.AUROWORK_INBOX_ENABLED ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

function resolveOutboxEnabled(): boolean {
  const raw = (process.env.AUROWORK_OUTBOX_ENABLED ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

function resolveInboxMaxBytes(): number {
  const raw = (process.env.AUROWORK_INBOX_MAX_BYTES ?? "").trim();
...
function resolveToyUiEnabled(): boolean {
  const raw = (process.env.AUROWORK_TOY_UI ?? "").trim().toLowerCase();
  if (!raw) return true;
  return ["1", "true", "yes", "on"].includes(raw);
}

function resolveBrowserProvider(): Capabilities["toolProviders"]["browser"] {
  const raw = (process.env.AUROWORK_BROWSER_PROVIDER ?? "").trim().toLowerCase();
...
function resolveInboxDir(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "aurowork", "inbox");
}

function resolveOutboxDir(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "aurowork", "outbox");
}

function resolveAgentLabDir(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "aurowork", "agentlab");
}
...
function createRoutes(
  ...
  addRoute(routes, "GET", "/ui/assets/aurowork-mark.svg", "none", async () => {
    if (!resolveToyUiEnabled()) {
      throw new ApiError(404, "ui_disabled", "Toy UI is disabled");
    }
  ...
  addRoute(routes, "PATCH", "/workspace/:id/config", "client", async (ctx) => {
    ...
    const aurowork = body.aurowork as Record<string, unknown> | undefined;

    if (!opencode && !aurowork) {
      throw new ApiError(400, "invalid_payload", "opencode or aurowork updates required");
    }
    ...
    await requireApproval(ctx, {
      ...
      paths: [opencode ? opencodeConfigPath(workspace.path) : null, aurowork ? auroworkConfigPath(workspace.path) : null].filter(Boolean) as string[],
    });
    ...
    if (aurowork) {
      await writeAuroworkConfig(workspace.path, aurowork, true);
    }
  ...
  addRoute(routes, "POST", "/workspace/:id/import", "client", async (ctx) => {
    ...
    await requireApproval(ctx, {
      ...
      paths: [
        ...
        auroworkConfigPath(workspace.path),
      ],
    });
...
function resolveOpenCodeRouterConfigPath(): string {
  ...
  const dataDir = process.env.OPENCODE_ROUTER_DATA_DIR?.trim() || join(homedir(), ".aurowork", "opencode-router");
...
type AuroworkServerConfigFile = Record<string, unknown> & {
  workspaces?: Array<Record<string, unknown>>;
  authorizedRoots?: string[];
};

async function readServerConfigFile(configPath: string): Promise<AuroworkServerConfigFile> {
  ...
  try {
    ...
    return ensurePlainObject(JSON.parse(raw)) as AuroworkServerConfigFile;
  } catch (error) {
...
async function persistServerWorkspaceState(config: ServerConfig): Promise<boolean> {
  ...
  const next: AuroworkServerConfigFile = {
    ...parsed,
...
function getRuntimeControlConfig(): { baseUrl: string; token: string } | null {
  const baseUrl = process.env.AUROWORK_CONTROL_BASE_URL?.trim() ?? "";
  const token = process.env.AUROWORK_CONTROL_TOKEN?.trim() ?? "";
...
async function readAuroworkConfig(workspaceRoot: string): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    throw new ApiError(422, "invalid_json", "Failed to parse aurowork.json");
  }
}
...
async function writeAuroworkConfig(workspaceRoot: string, payload: Record<string, unknown>, merge: boolean): Promise<void> {
  const path = auroworkConfigPath(workspaceRoot);
  const next = merge ? { ...(await readAuroworkConfig(workspaceRoot)), ...payload } : payload;
...
async function importWorkspace(workspace: WorkspaceInfo, payload: Record<string, unknown>): Promise<void> {
  ...
  const aurowork = payload.aurowork as Record<string, unknown> | undefined;
  ...
  if (aurowork) {
    if (modes.aurowork === "replace") {
      await writeAuroworkConfig(workspace.path, aurowork, false);
    } else {
      await writeAuroworkConfig(workspace.path, aurowork, true);
    }
  }
