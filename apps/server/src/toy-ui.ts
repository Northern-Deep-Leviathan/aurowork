...
<html lang="en">
  <head>
    ...
    <title>AuroWork Toy UI</title>
    <link rel="icon" type="image/svg+xml" href="/ui/assets/aurowork-mark.svg" />
  ...
  <body>
    <div class="wrap">
      <div class="top">
        <div class="title">
          <h1>AuroWork Toy UI</h1>
          <div class="sub">Local-first host contract harness (served by aurowork-server)</div>
        </div>
        <div class="row">
          ...
          <div class="body">
            ...
            <div class="panel hidden" data-panel="automations">
              <div class="row">
                ...
                <span class="small">Apply schedule on host via <span class="mono">aurowork-agent-lab scheduler sync</span>.</span>
              </div>
            ...
            <div class="panel hidden" data-panel="config">
              ...
              <div class="small">Uploads go to <span class="mono">.opencode/aurowork/inbox/</span> inside the workspace.</div>

              <div class="hr"></div>

              <div class="row">
                <button class="btn" id="btn-artifacts">List artifacts</button>
                <span class="small">Downloads read from <span class="mono">.opencode/aurowork/outbox/</span>.</span>
              </div>
...
const STORAGE_TOKEN = "aurowork.toy.token";
const STORAGE_SESSION_PREFIX = "aurowork.toy.session.";
...
async function ensureSession(workspaceId) {
  ...
  const created = await apiFetch("/w/" + encodeURIComponent(workspaceId) + "/opencode/session", {
    method: "POST",
    body: JSON.stringify({ title: "AuroWork Toy UI" }),
  });
...
async function main() {
  ...
  qs("#btn-delete-workspace").onclick = async () => {
    if (!confirm("Delete this workspace from the host's AuroWork server config?")) return;
    try {
