...
const useCorepackPnpm = process.platform === "win32" && process.env.AUROWORK_USE_COREPACK_PNPM === "1";
...
const hostOverride = process.env.AUROWORK_DEV_HOST?.trim() || null;
const port = readPort();
...
const runPrepareSidecars = () => {
  ...
  if (process.env.AUROWORK_SIDECAR_FORCE_BUILD !== "0") {
    args.push("--force");
  }
...
const ensureLinuxDesktopDependencies = () => {
  ...
  console.error(
    "[aurowork] Missing Linux desktop system dependencies required by Tauri:\n" +
      `  ${missing.join(", ")}\n\n` +
...
const main = async () => {
  ...
  if (detectedViteUrl) {
    console.log(`[aurowork] UI dev server already running at ${detectedViteUrl} (reusing).`);
    holdOpenUntilSignal();
    return;
  }
  ...
  if (portInUse) {
    console.error(
      `[aurowork] Port ${port} is in use, but it does not look like a Vite dev server.\n` +
        `Set PORT to a free port (e.g. PORT=5174) or stop the process using port ${port}.`
    );
    process.exit(1);
  }

  console.log(`[aurowork] Starting UI dev server on port ${port}...`);
