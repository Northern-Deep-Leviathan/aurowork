...
pub fn resolve_orchestrator_data_dir() -> String {
    let env_dir = env::var("AUROWORK_DATA_DIR")
        .ok()
    ...
    if let Some(home) = home_dir() {
        return home
            .join(".aurowork")
            .join("aurowork-orchestrator")
            .to_string_lossy()
            .to_string();
    }

    ".aurowork/aurowork-orchestrator".to_string()
}

fn orchestrator_state_path(data_dir: &str) -> PathBuf {
    Path::new(data_dir).join("aurowork-orchestrator-state.json")
}

fn orchestrator_auth_path(data_dir: &str) -> PathBuf {
    Path::new(data_dir).join("aurowork-orchestrator-auth.json")
}
...
pub fn spawn_orchestrator_daemon(
    ...
    let command = match app.shell().sidecar("aurowork-orchestrator") {
        Ok(command) => command,
        Err(_) => app.shell().command("aurowork"),
    };
    ...
    command = command.env("AUROWORK_INTERNAL_ALLOW_OPENCODE_CREDENTIALS", "1");
...
mod tests {
    ...
    fn request_shutdown_returns_false_without_state() {
        let dir = std::env::temp_dir().join(format!(
            "aurowork-orchestrator-shutdown-missing-{}",
            Uuid::new_v4()
        ));
    ...
    fn request_shutdown_posts_to_daemon_endpoint() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "aurowork-orchestrator-shutdown-state-{}",
            Uuid::new_v4()
        ));
        fs::create_dir_all(&dir).expect("create state dir");
        let state_path = dir.join("aurowork-orchestrator-state.json");
        fs::write(
            &state_path,
            format!(
