...
1) Install the AuroWork-pinned OpenCode version
...
fn seed_aurowork_agent(agent_root: &PathBuf) -> Result<(), String> {
    let agent_path = agent_root.join("aurowork.md");
...
pub fn ensure_workspace_files(workspace_path: &str, preset: &str) -> Result<(), String> {
    ...
    seed_aurowork_agent(&agents_dir)?;
    ...
    if let Some(obj) = config.as_object_mut() {
        ...
        if current.is_empty() {
            obj.insert(
                ...
                serde_json::Value::String("aurowork".to_string()),
            );
            config_changed = true;
        }
    }
    ...
    let aurowork_path = root.join(".opencode").join("aurowork.json");
    if !aurowork_path.exists() {
        let aurowork = WorkspaceAuroworkConfig::new(workspace_path, preset, now_ms());

        fs::create_dir_all(aurowork_path.parent().unwrap())
            .map_err(|e| format!("Failed to create {}: {e}", aurowork_path.display()))?;

        fs::write(
            &aurowork_path,
            serde_json::to_string_pretty(&aurowork).map_err(|e| e.to_string())?,
        )
