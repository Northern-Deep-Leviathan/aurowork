...
fn aurowork_dev_mode_enabled() -> bool {
...
pub fn engine_install() -> Result<ExecResult, String> {
    #[cfg(windows)]
    {
      ...
      status: -1,
      stdout: String::new(),
      stderr: "Guided install is not supported on Windows yet. Install the AuroWork-pinned OpenCode version manually, then restart AuroWork.".to_string(),
    });
    }
...
pub fn engine_start(
    ...
    if runtime == EngineRuntime::Orchestrator {
        ...
        // aurowork-orchestrator doesn't start its daemon HTTP server until it has ensured that
        ...
        let health_timeout_ms = std::env::var("AUROWORK_ORCHESTRATOR_START_TIMEOUT_MS")
            .ok()
