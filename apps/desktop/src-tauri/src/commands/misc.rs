...
use crate::types::{ExecResult, WorkspaceAuroworkConfig};
...
fn current_aurowork_state_paths(app: &AppHandle) -> Result<Vec<PathBuf>, String> {
    Ok(vec![
        app.path()
            .app_cache_dir()
...
fn stop_host_services(
    ...
    if let Ok(mut aurowork_state) = aurowork_manager.inner.lock() {
        AuroworkServerManager::stop_locked(&mut aurowork_state);
    }
}
...
fn read_workspace_aurowork_config(
    workspace_path: &Path,
) -> Result<WorkspaceAuroworkConfig, String> {
    let aurowork_path = workspace_path.join(".opencode").join("aurowork.json");
    if !aurowork_path.exists() {
        let mut cfg = WorkspaceAuroworkConfig::default();
    ...
    let raw = fs::read_to_string(&aurowork_path)
        .map_err(|e| format!("Failed to read {}: {e}", aurowork_path.display()))?;

    serde_json::from_str::<WorkspaceAuroworkConfig>(&raw)
        .map_err(|e| format!("Failed to parse {}: {e}", aurowork_path.display()))
}
...
fn load_authorized_roots(app: &AppHandle) -> Result<Vec<PathBuf>, String> {
    ...
    for workspace in state.workspaces {
        ...
        let mut config = read_workspace_aurowork_config(&workspace_path)?;
...
pub fn app_build_info(app: AppHandle) -> AppBuildInfo {
    ...
    let git_sha = option_env!("AUROWORK_GIT_SHA").map(|value| value.to_string());
    let build_epoch = option_env!("AUROWORK_BUILD_EPOCH").map(|value| value.to_string());
    AppBuildInfo {
        version,
        git_sha,
        build_epoch,
        aurowork_dev_mode: env_truthy("AUROWORK_DEV_MODE"),
    }
}
...
pub fn nuke_aurowork_and_opencode_config_and_exit(
    ...
    if dev_mode {
        ...
        // by the dev app identity and AUROWORK_DATA_DIR, so only clear those dev paths.
    } else {
