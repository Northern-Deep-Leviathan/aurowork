...
pub fn workspace_register(
    ...
    println!("[workspace] register request");
    ...
    let preset = if aurowork_path.exists() {
        ...
        serde_json::from_str::<WorkspaceAuroworkConfig>(&raw)
            .ok()
            .and_then(|cfg| cfg.workspace.and_then(|w| w.preset))
    ...
    save_workspace_state(&app, &state)?;
    println!("[workspace] register complete: {id}");
...
pub fn workspace_add_authorized_root(
    ...
    let aurowork_path = PathBuf::from(&workspace_path)
        .join(".opencode")
        .join("aurowork.json");

    if let Some(parent) = aurowork_path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Failed to create {}: {e}", parent.display()))?;
    }

    let mut config: WorkspaceAuroworkConfig = if aurowork_path.exists() {
    ...
    .map_err(|e| format!("Failed to write {}: {e}", aurowork_path.display()))?;
...
#[tauri::command]
pub fn workspace_aurowork_read(
    _app: tauri::AppHandle,
    workspace_path: String,
...
#[tauri::command]
pub fn workspace_aurowork_write(
    _app: tauri::AppHandle,
    workspace_path: String,
    ...
    Ok(ExecResult {
        ok: true,
        status: 0,
        stdout: format!("Wrote {}", aurowork_path.display()),
        stderr: String::new(),
    })
}
...
pub fn workspace_import_config(
    ...
    let aurowork_path = target_path.join(".opencode").join("aurowork.json");
    ...
    if aurowork_path.exists() {
        let raw = fs::read_to_string(&aurowork_path)
            .map_err(|e| format!("Failed to read {}: {e}", aurowork_path.display()))?;
        if let Ok(mut config) = serde_json::from_str::<WorkspaceAuroworkConfig>(&raw) {
        ...
        let config = WorkspaceAuroworkConfig::new(&target_dir, &preset, now_ms());
        if let Some(parent) = aurowork_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| format!("Failed to create {}: {e}", parent.display()))?;
        }
