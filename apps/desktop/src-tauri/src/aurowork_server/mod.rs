...
use manager::AuroworkServerManager;
use spawn::{resolve_aurowork_port, spawn_aurowork_server};
...
fn aurowork_server_token_store_path(app: &AppHandle) -> Result<PathBuf, String> {
    ...
    Ok(data_dir.join("aurowork-server-tokens.json"))
}
...
fn aurowork_server_state_path(app: &AppHandle) -> Result<PathBuf, String> {
    let data_dir = app
        ...
        .app_data_dir()
        .map_err(|e| format!("Failed to resolve app data dir: {e}"))?;
    Ok(data_dir.join("aurowork-server-state.json"))
}

fn load_aurowork_server_token_store(
    path: &Path,
) -> Result<PersistedAuroworkServerTokenStore, String> {
    if !path.exists() {
        return Ok(PersistedAuroworkServerTokenStore {
            version: AUROWORK_SERVER_TOKEN_STORE_VERSION,
    ...
    let mut store: PersistedAuroworkServerTokenStore = serde_json::from_str(&raw)
        .map_err(|e| format!("Failed to parse {}: {e}", path.display()))?;
    if store.version < AUROWORK_SERVER_TOKEN_STORE_VERSION {
        store.version = AUROWORK_SERVER_TOKEN_STORE_VERSION;
    }
    Ok(store)
}

fn save_aurowork_server_token_store(
    path: &Path,
    store: &PersistedAuroworkServerTokenStore,
) -> Result<(), String> {
...
fn load_aurowork_server_state(path: &Path) -> Result<PersistedAuroworkServerState, String> {
    if !path.exists() {
        return Ok(PersistedAuroworkServerState {
            version: AUROWORK_SERVER_STATE_VERSION,
    ...
    let raw =
        fs::read_to_string(path).map_err(|e| format!("Failed to read {}: {e}", path.display()))?;
    let mut state: PersistedAuroworkServerState = serde_json::from_str(&raw)
        .map_err(|e| format!("Failed to parse {}: {e}", path.display()))?;
    if state.version < AUROWORK_SERVER_STATE_VERSION {
        if state.version < 2 && state.preferred_port == Some(LEGACY_FIXED_AUROWORK_PORT) {
            state.preferred_port = None;
        }
        if state.version < 3 && state.preferred_port == Some(LEGACY_FIXED_AUROWORK_PORT) {
            state.preferred_port = None;
        }
        state.version = AUROWORK_SERVER_STATE_VERSION;
    }
    Ok(state)
}

fn save_aurowork_server_state(
    path: &Path,
    state: &PersistedAuroworkServerState,
) -> Result<(), String> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Failed to create {}: {e}", parent.display()))?;
...
fn persist_preferred_aurowork_port(
    app: &AppHandle,
    workspace_key: &str,
    port: u16,
) -> Result<(), String> {
    let path = aurowork_server_state_path(app)?;
    let mut state = load_aurowork_server_state(&path)?;
    state.version = AUROWORK_SERVER_STATE_VERSION;
    let trimmed = workspace_key.trim();
    if trimmed.is_empty() {
    ...
    save_aurowork_server_state(&path, &state)
}
...
fn load_or_create_workspace_tokens(
    ...
    let path = aurowork_server_token_store_path(app)?;
...
fn load_or_create_workspace_tokens_at_path(
    ...
    let mut store = load_aurowork_server_token_store(path)?;
    if let Some(tokens) = store.workspaces.get(workspace_key) {
        return Ok(tokens.clone());
    }

    let tokens = PersistedAuroworkServerTokens {
...
fn wait_for_aurowork_health(base_url: &str, timeout: Duration) -> Result<(), String> {
    ...
    while Instant::now() < deadline {
        match ureq::get(&health_url).call() {
            ...
            Ok(response) => {
                last_error = format!(
                    "AuroWork server health check returned {}",
                    response.status()
                )
            }
...
fn issue_owner_token(base_url: &str, host_token: &str) -> Result<String, String> {
    let response = ureq::post(&format!("{}/tokens", base_url.trim_end_matches('/')))
        ...
        .send_string(r#"{"scope":"owner","label":"AuroWork desktop owner token"}"#)
...
pub fn start_aurowork_server(
    ...
    let active_workspace = workspace_paths
        ...
        .unwrap_or("");
    let preferred_port = read_preferred_aurowork_port(app, active_workspace)?;
    let reserved_ports = reserved_aurowork_ports(app, active_workspace)?;
    let port = resolve_aurowork_port(&host, preferred_port, &reserved_ports)?;
    ...
    let (mut rx, child) = spawn_aurowork_server(
        app,
        &host,
        port,
        workspace_paths,
                ...
                CommandEvent::Terminated(payload) => {
                    if let Ok(mut state) = state_handle.try_lock() {
                        ...
                        if let Some(code) = payload.code {
                            let next = format!("AuroWork server exited (code {code}).");
                            state.last_stderr = Some(truncate_output(&next, 8000));
                        }
                    }
                }
    ...
    Ok(AuroworkServerManager::snapshot_locked(&mut state))
}
...
mod tests {
    ...
    fn unique_temp_path(name: &str) -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("aurowork-server-{name}-{nonce}.json"))
    }
