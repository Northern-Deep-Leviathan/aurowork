...
fn emit_build_info() {
    let sha = env::var("AUROWORK_GIT_SHA")
        .ok()
        .and_then(|value| {
    ...
    if let Some(value) = sha {
        println!("cargo:rustc-env=AUROWORK_GIT_SHA={}", value);
    }
    ...
    if let Some(value) = build_epoch {
        println!("cargo:rustc-env=AUROWORK_BUILD_EPOCH={}", value);
    }
}
...
fn ensure_orchestrator_sidecar() {
    ...
    let canonical_name = if target.contains("windows") {
        "aurowork-orchestrator.exe"
    } else {
        "aurowork-orchestrator"
    };
    let mut target_name = format!("aurowork-orchestrator-{target}");
    if target.contains("windows") {
        target_name.push_str(".exe");
    }
    ...
    let Some(source_path) = source_path else {
        println!(
            "cargo:warning=orchestrator sidecar missing at {} (set AUROWORK_ORCHESTRATOR_BIN_PATH or install aurowork-orchestrator)",
            dest_path.display()
        );
        create_debug_stub(&dest_path, &sidecar_dir, &profile, &target);
        create_debug_stub(&target_dest_path, &sidecar_dir, &profile, &target);
...
fn ensure_aurowork_server_sidecar() {
    let target = env::var("CARGO_CFG_TARGET_TRIPLE")
        .or_else(|_| env::var("TARGET"))
        .or_else(|_| env::var("TAURI_ENV_TARGET_TRIPLE"))
        .unwrap_or_default();
    ...
    let mut target_name = format!("aurowork-server-{target}");
    if target.contains("windows") {
        target_name.push_str(".exe");
    }
    ...
    let Some(source_path) = source_path else {
        println!(
      "cargo:warning=AuroWork server sidecar missing at {} (set AUROWORK_SERVER_BIN_PATH or install aurowork-server)",
      dest_path.display()
    );
    ...
    if copied {
        ...
        println!(
            "cargo:warning=Failed to copy AuroWork server sidecar from {} to {}",
            source_path.display(),
            dest_path.display()
        );
        create_debug_stub(&dest_path, &sidecar_dir, &profile, &target);
