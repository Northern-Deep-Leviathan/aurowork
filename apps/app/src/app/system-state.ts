...
export function createSystemState(options: {
  ...
  const [resetModalMode, setResetModalMode] = createSignal<ResetAuroworkMode>("onboarding");
  ...
  function clearAuroworkLocalStorage(mode: ResetAuroworkMode) {
    ...
    try {
      ...
      for (const key of keys) {
        if (key.includes("aurowork")) {
          window.localStorage.removeItem(key);
        }
      }
      // Legacy compatibility key
      window.localStorage.removeItem("aurowork_mode_pref");
    } catch {
      // ignore
    }
  }

  function openResetModal(mode: ResetAuroworkMode) {
    if (anyActiveRuns()) {
  ...
  async function confirmReset() {
    ...
    try {
      if (isTauriRuntime()) {
        await resetAuroworkState(resetModalMode());
      }

      clearAuroworkLocalStorage(resetModalMode());
  ...
  const reloadCopy = createMemo(() => {
    ...
    if (!reasons.length) {
      return {
        ...
        body: "AuroWork detected changes that require reloading the OpenCode instance.",
      };
    }
    ...
    return {
      title: "Reload required",
      body: "AuroWork detected OpenCode configuration changes. Reload the engine to apply them.",
    };
  });
  ...
  async function reloadEngineInstance() {
    ...
    try {
      ...
      if (options.notion) {
        ...
        try {
          window.localStorage.setItem("aurowork.notionStatus", nextStatus);
          if (nextStatus === "connected") {
            const detail = options.notion.statusDetail();
            if (detail) {
              window.localStorage.setItem("aurowork.notionStatusDetail", detail);
            } else {
              window.localStorage.removeItem("aurowork.notionStatusDetail");
            }
          }
        } catch {
          // ignore
  ...
  async function cleanupAuroworkDockerContainers() {
    if (!isTauriRuntime()) {
    ...
    try {
      ...
      const result = { candidates: [] as string[], removed: [] as string[], errors: [] as string[] };
      if (!result.candidates.length) {
        setDockerCleanupResult("No AuroWork Docker containers found.");
        return;
      }
      ...
      setDockerCleanupResult(`Removed ${removedCount} AuroWork Docker container(s).`);
    } catch (e) {
