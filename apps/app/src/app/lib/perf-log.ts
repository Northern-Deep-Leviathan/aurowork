...
type PerfRoot = typeof globalThis & {
  __auroworkPerfSeq?: number;
  __auroworkPerfLogs?: PerfLogRecord[];
  __auroworkPerfConsoleAt?: Record<string, number>;
  __auroworkPerfConsoleSuppressed?: Record<string, number>;
};
...
export const recordPerfLog = (
  ...
  const id = (root.__auroworkPerfSeq ?? 0) + 1;
  root.__auroworkPerfSeq = id;
  ...
  const logs = root.__auroworkPerfLogs ?? [];
  logs.push(entry);
  if (logs.length > PERF_LOG_LIMIT) {
    logs.splice(0, logs.length - PERF_LOG_LIMIT);
  }
  root.__auroworkPerfLogs = logs;

  try {
    const key = `${scope}:${event}`;
    const now = Date.now();
    const lastByKey = root.__auroworkPerfConsoleAt ?? (root.__auroworkPerfConsoleAt = {});
    const suppressedByKey =
      root.__auroworkPerfConsoleSuppressed ?? (root.__auroworkPerfConsoleSuppressed = {});
...
export const clearPerfLogs = () => {
  const root = globalThis as PerfRoot;
  root.__auroworkPerfLogs = [];
  root.__auroworkPerfSeq = 0;
};
