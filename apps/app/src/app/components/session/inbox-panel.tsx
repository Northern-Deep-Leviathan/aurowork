...
import type { AuroworkInboxItem, AuroworkServerClient } from "../../lib/aurowork-server";
import WebUnavailableSurface from "../web-unavailable-surface";
import { formatBytes, formatRelativeTime } from "../../utils";

export type InboxPanelProps = {
  id?: string;
  client: AuroworkServerClient | null;
  workspaceId: string | null;
  onToast?: (message: string) => void;
  maxPreview?: number;
};

const INBOX_PREFIX = ".opencode/aurowork/inbox/";

function safeName(item: AuroworkInboxItem): string {
  return String(item.name ?? item.path ?? "file").trim() || "file";
}

function safeRelPath(item: AuroworkInboxItem): string {
  const raw = String(item.path ?? item.name ?? "").trim();
  if (!raw) return "";
  return raw.replace(/^\/+/, "");
}

function toInboxWorkspacePath(item: AuroworkInboxItem): string {
...
export default function InboxPanel(props: InboxPanelProps) {
  const [items, setItems] = createSignal<AuroworkInboxItem[]>([]);
  ...
  const copyPath = async (item: AuroworkInboxItem) => {
  ...
  const downloadItem = async (item: AuroworkInboxItem) => {
    const client = props.client;
    const workspaceId = (props.workspaceId ?? "").trim();
    if (!client || !workspaceId) {
