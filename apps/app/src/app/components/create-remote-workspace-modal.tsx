...
export default function CreateRemoteWorkspaceModal(props: {
  ...
  const [auroworkHostUrl, setAuroworkHostUrl] = createSignal("");
  ...
  const canSubmit = createMemo(() => {
    ...
    return auroworkHostUrl().trim().length > 0;
  });
  ...
  createEffect(() => {
    ...
    setAuroworkHostUrl(defaults.auroworkHostUrl?.trim() ?? "");
    setAuroworkToken(defaults.auroworkToken?.trim() ?? "");
    setAuroworkTokenVisible(false);
        ...
        <div class="rounded-xl border border-dls-border bg-dls-sidebar px-5 py-4">
          <div class="mb-4 flex items-start gap-3">
            ...
            <div class="min-w-0">
              ...
              <div class="mt-1 text-[13px] leading-6 text-dls-secondary">Use the URL your AuroWork server shared with you. Add a token only if the server needs one.</div>
            </div>
          </div>

          <div class="grid gap-4">
            <label class="grid gap-2">
              <span class={fieldLabelClass}>{translate("dashboard.aurowork_host_label")}</span>
              <input
                ref={inputRef}
                type="url"
                value={auroworkHostUrl()}
                onInput={(event) => setAuroworkHostUrl(event.currentTarget.value)}
                placeholder={translate("dashboard.aurowork_host_placeholder")}
                disabled={submitting()}
                class={fieldInputClass}
              />
              <span class={fieldHintClass}>{translate("dashboard.aurowork_host_hint")}</span>
            </label>

            <label class="grid gap-2">
              <span class={fieldLabelClass}>{translate("dashboard.aurowork_host_token_label")}</span>
              ...
              <span class={fieldHintClass}>{translate("dashboard.aurowork_host_token_hint")}</span>
            </label>
      ...
      <div class="space-y-3 border-t border-dls-border bg-dls-surface px-6 py-5">
        ...
        <div class="flex justify-end gap-3">
          ...
          <button
            ...
            onClick={() =>
              props.onConfirm({
                auroworkHostUrl: auroworkHostUrl().trim(),
                auroworkToken: auroworkToken().trim(),
                directory: directory().trim() ? directory().trim() : null,
                displayName: displayName().trim() ? displayName().trim() : null,
              })
            }
            disabled={!canSubmit()}
            title={!auroworkHostUrl().trim() ? translate("dashboard.remote_base_url_required") : undefined}
