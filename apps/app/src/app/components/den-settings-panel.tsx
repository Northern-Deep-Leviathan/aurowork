...
export default function DenSettingsPanel(props: DenSettingsPanelProps) {
  ...
  const openBrowserAuth = (mode: "sign-in" | "sign-up") => {
    ...
    setStatusMessage(
      mode === "sign-up"
        ? "Finish account creation in your browser to connect AuroWork."
        : "Finish signing in in your browser to connect AuroWork.",
    );
    setAuthError(null);
  };
  ...
  const parseManualAuthInput = (value: string) => {
    ...
    try {
      ...
      if (
        (protocol === "aurowork:" || protocol === "aurowork-dev:") &&
  ...
  const submitManualAuth = async () => {
    ...
    if (!parsed || authBusy()) {
      if (!parsed) {
        setAuthError("Paste a valid AuroWork sign-in link or one-time sign-in code.");
      }
      return;
    }
    ...
    setAuthBusy(true);
    setAuthError(null);
    setStatusMessage("Finishing AuroWork Cloud sign-in...");
          ...
          detail: {
            ...
            message:
              error instanceof Error
                ? error.message
                : "Failed to complete AuroWork Cloud sign-in.",
          },
        }),
      );
    } finally {
      ...
      if (customEvent.detail?.status === "success") {
        ...
        setStatusMessage(
          customEvent.detail.email?.trim()
            ? `Connected AuroWork Cloud as ${customEvent.detail.email.trim()}.`
            : "Connected AuroWork Cloud.",
        );
      } else if (customEvent.detail?.status === "error") {
        setAuthError(
          customEvent.detail.message?.trim() ||
            "Failed to finish AuroWork Cloud sign-in.",
        );
      }
    };
  ...
  const signOut = async () => {
    ...
    clearSignedInState(
      "Signed out and cleared your AuroWork Cloud session on this device.",
    );
  };

  const handleAuroWorker = async (workerId: string, workerName: string) => {
    const orgId = activeOrgId().trim();
    if (!orgId) {
    ...
    try {
      ...
      const auroworkUrl = tokens.auroworkUrl?.trim() ?? "";
      const accessToken =
      ...
      if (!ok) {
        throw new Error(`Failed to open ${workerName} in AuroWork.`);
      }

      setStatusMessage(`Opened ${workerName} in AuroWork.`);
    } catch (error) {
      setWorkersError(
          ...
          <div class="space-y-2">
            ...
            <div>
              ...
              <div class="mt-1 max-w-[60ch] text-xs text-dls-secondary">
                Sign in to AuroWork Cloud to keep your tasks alive even when your
                computer sleeps.
              </div>
            </div>
          </div>
          ...
          <Show when={manualAuthOpen()}>
            <div class={`${settingsPanelSoftClass} space-y-3`}>
              <TextInput
                ...
                placeholder="aurowork://den-auth?... or pasted code"
                disabled={authBusy() || sessionBusy()}
                hint="If your browser doesn't bounce back into AuroWork automatically, paste the sign-in link or one-time code from AuroWork Cloud here."
              />
              <div class="flex flex-wrap items-center gap-2">
                <Button
                  variant="secondary"
                ...
                <div class="text-[11px] text-dls-secondary">
                  Accepts an <span class="font-mono">aurowork://den-auth</span> link or the raw one-time grant.
                </div>
              </div>
            </div>
          </Show>
          ...
          <div class={`${settingsPanelClass} space-y-4`}>
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                ...
                <div class="mt-1 text-xs text-dls-secondary">
                  Open workers directly into AuroWork using the same
                  ...
                  return (
                    <div class="flex items-center justify-between rounded-xl px-3 py-2 text-left text-[13px] transition-colors hover:bg-gray-2/60">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          void handleAuroWorker(worker.workerId, worker.workerName)
                        }
