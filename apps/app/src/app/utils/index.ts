...
const STARTUP_PREF_KEY = "aurowork.startupPref";
const LEGACY_PREF_KEY = "aurowork.modePref";
const LEGACY_PREF_KEY_ALT = "aurowork_mode_pref";
...
export function getWorkspaceTaskLoadErrorDisplay(workspace: WorkspaceInfo, error?: string | null) {
  ...
  const host = `${workspace.baseUrl ?? ""} ${workspace.auroworkHostUrl ?? ""}`.toLowerCase();
...
const TRUNCATED_SYSTEM_PATH_PATTERNS = [
  /com\.[^/]+\.(aurowork|opencode)/i, // macOS app bundle identifiers
  /\.aurowork\.dev\//i, // AuroWork dev paths
