...
export function createExtensionsStore(options: {
  ...
  const HUB_REPOS_STORAGE_KEY = "aurowork.skills.hubRepos.v1";
  ...
  async function refreshHubSkills(optionsOverride?: { force?: boolean }) {
    ...
    const canUseAuroworkServer =
      options.auroworkServerStatus() === "connected" &&
      auroworkClient &&
      auroworkCapabilities?.hub?.skills?.read &&
      typeof (auroworkClient as any).listHubSkills === "function";
    ...
    try {
      ...
      if (canUseAuroworkServer) {
        const response = await (auroworkClient as any).listHubSkills({
          repo: {
            owner: repo.owner,
            repo: repo.repo,
            ref: repo.ref,
  ...
  async function installHubSkill(name: string): Promise<{ ok: boolean; message: string }> {
    ...
    const canUseAuroworkServer =
      ...
      auroworkClient &&
      auroworkWorkspaceId &&
      auroworkCapabilities?.hub?.skills?.install &&
      typeof (auroworkClient as any).installHubSkill === "function";

    if (!canUseAuroworkServer) {
      if (isRemoteWorkspace) {
        return { ok: false, message: "AuroWork server unavailable. Connect to install skills." };
      }
      return { ok: false, message: "Hub install requires AuroWork server." };
    }
    ...
    try {
      const repoOverride: AuroworkHubRepo = {
        owner: repo.owner,
        repo: repo.repo,
        ref: repo.ref,
      };
      const result = await (auroworkClient as any).installHubSkill(auroworkWorkspaceId, trimmed, {
        repo: repoOverride,
      });
  ...
  async function refreshSkills(optionsOverride?: { force?: boolean }) {
    ...
    const canUseAuroworkServer =
      ...
      auroworkClient &&
      auroworkWorkspaceId &&
      auroworkCapabilities?.skills?.read;
    ...
    // Prefer AuroWork server when available
    if (canUseAuroworkServer) {
      if (root !== skillsRoot) {
        skillsLoaded = false;
      }
      ...
      try {
        setSkillsStatus(null);
        const response = await auroworkClient.listSkills(auroworkWorkspaceId, {
    ...
    if (!c) {
      setSkills([]);
      setSkillsStatus("AuroWork server unavailable. Connect to load skills.");
      return;
    }
  ...
  async function refreshPlugins(scopeOverride?: PluginScope) {
    ...
    const canUseAuroworkServer =
      ...
      auroworkClient &&
      auroworkWorkspaceId &&
      auroworkCapabilities?.plugins?.read;
    ...
    if (scope === "project" && canUseAuroworkServer) {
      setPluginConfig(null);
      setPluginConfigPath(`opencode.json (${isRemoteWorkspace ? "remote" : "aurowork"} server)`);
      ...
      try {
        ...
        const result = await auroworkClient.listPlugins(auroworkWorkspaceId, { includeGlobal: false });
        if (refreshPluginsAborted) return;
    ...
    if (!isLocalWorkspace && !canUseAuroworkServer) {
      setPluginStatus("AuroWork server unavailable. Connect to manage plugins.");
      setPluginList([]);
      setSidebarPluginStatus("Connect an AuroWork server to load plugins.");
      setSidebarPluginList([]);
      refreshPluginsInFlight = false;
      return;
    }
  ...
  async function addPlugin(pluginNameOverride?: string) {
    ...
    const canUseAuroworkServer =
      ...
      auroworkClient &&
      auroworkWorkspaceId &&
      auroworkCapabilities?.plugins?.write;
    ...
    if (pluginScope() === "project" && canUseAuroworkServer) {
      try {
        setPluginStatus(null);
        await auroworkClient.addPlugin(auroworkWorkspaceId, pluginName);
        if (isManualInput) {
          setPluginInput("");
        }
  ...
  async function removePlugin(pluginName: string) {
    ...
    if (pluginScope() === "project" && canUseAuroworkServer) {
      try {
        setPluginStatus(null);
        await auroworkClient.removePlugin(auroworkWorkspaceId, name);
        await refreshPlugins("project");
      } catch (e) {
  ...
  async function installSkillCreator(): Promise<{ ok: boolean; message: string }> {
    ...
    const canUseAuroworkServer =
      ...
      auroworkClient &&
      auroworkWorkspaceId &&
      auroworkCapabilities?.skills?.write;

    // Use AuroWork server when available
    if (canUseAuroworkServer) {
      options.setBusy(true);
      options.setError(null);
      ...
      try {
        await auroworkClient.upsertSkill(auroworkWorkspaceId, {
          name: "skill-creator",
    ...
    if (isRemoteWorkspace) {
      const message = "AuroWork server unavailable. Connect to install skills.";
      setSkillsStatus(message);
      return { ok: false, message };
    }
  ...
  async function readSkill(name: string): Promise<{ name: string; path: string; content: string } | null> {
    ...
    const canUseAuroworkServer =
      ...
      auroworkWorkspaceId &&
      auroworkCapabilities?.skills?.read &&
      typeof (auroworkClient as any).getSkill === "function";

    if (canUseAuroworkServer) {
      try {
        setSkillsStatus(null);
        const result = await (auroworkClient as AuroworkServerClient & { getSkill: any }).getSkill(
          auroworkWorkspaceId,
          trimmed,
          { includeGlobal: isLocalWorkspace },
        );
    ...
    if (isRemoteWorkspace) {
      setSkillsStatus("AuroWork server unavailable. Connect to view skills.");
      return null;
    }
  ...
  async function saveSkill(input: { name: string; content: string; description?: string }) {
    ...
    if (isRemoteWorkspace) {
      setSkillsStatus("AuroWork server unavailable. Connect to edit skills.");
      return;
    }
