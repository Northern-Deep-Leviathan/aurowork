...
import { abortSessionSafe } from "../lib/auro-session";
...
export function createSessionStore(options: {
  ...
  const auroworkConfigPattern = /[\\/]\.opencode[\\/]aurowork\.json\b/i;
  ...
  const detectReloadReason = (value: unknown): ReloadReason | null => {
    ...
    if (!text) return null;
    if (auroworkConfigPattern.test(text)) return null;
  ...
  const detectReloadTriggerFromText = (text: string): ReloadTrigger | null => {
    if (auroworkConfigPattern.test(text)) {
      return null;
    }
  ...
  const maybeAbortSyntheticControlLoop = (part: Part) => {
    ...
    const message =
      kind === "task-summary"
        ? "AuroWork stopped this run after detecting a likely synthetic task-summary loop. The engine kept asking itself to summarize task output and continue, which can repeat Goal/Instructions/Discoveries summaries without making progress."
        : "AuroWork stopped this run after detecting a likely auto-compaction continuation loop. The engine kept injecting synthetic continue prompts after compaction, which can burn tokens without advancing the task.";
  ...
  async function loadSessions(scopeRoot?: string) {
    ...
    // When no root is provided, return an empty list instead of all sessions to
    // prevent orphan sessions from surfacing after all workspaces are removed.
  ...
  let _debugFileLastDelta = 0;
  let _debugFileDeltaCount = 0;

  const applyEvent = async (event: OpencodeEvent) => {
    // File-log all SSE events (throttle high-frequency deltas)
    if (event.type === "message.part.delta") {
      _debugFileDeltaCount++;
      const now = Date.now();
      if (now - _debugFileLastDelta > 2000) {
        debugFileLog("sse", "message.part.delta", { batchCount: _debugFileDeltaCount });
        _debugFileDeltaCount = 0;
        _debugFileLastDelta = now;
      }
    } else if (event.type !== "message.part.updated") {
      // Log all non-delta, non-part-updated events
      const sessionID = (event.properties as Record<string, unknown>)?.sessionID ?? null;
      debugFileLog("sse", event.type, { sessionID });
    }
    ...
    if (event.type === "session.status") {
      if (event.properties && typeof event.properties === "object") {
        ...
        if (sessionID) {
          ...
          debugFileLog("sse", "session.status", { sessionID, raw: record.status, normalized });
    ...
    if (event.type === "session.idle") {
      if (event.properties && typeof event.properties === "object") {
        const record = event.properties as Record<string, unknown>;
        const sessionID = typeof record.sessionID === "string" ? record.sessionID : null;
        debugFileLog("sse", "session.idle", { sessionID });
        if (sessionID) {
          setStore("sessionStatus", sessionID, "idle");
          const c = options.client();
          if (c) {
    ...
    if (event.type === "session.error") {
      if (event.properties && typeof event.properties === "object") {
        const record = event.properties as Record<string, unknown>;
        const sessionID = typeof record.sessionID === "string" ? record.sessionID : null;
        debugFileLog("sse", "session.error", { sessionID, error: record.error });
        if (sessionID) {
          setStore("sessionStatus", sessionID, "idle");
        }
  ...
  createEffect(() => {
    ...
    const connectSse = async (controller: AbortController) => {
      try {
        ...
        debugFileLog("sse", "stream-error", { error: message });
        scheduleReconnect(controller);
      }
    };
    ...
    const scheduleReconnect = (oldController: AbortController) => {
      ...
      debugFileLog("sse", "reconnect-scheduled", { attempt: reconnectAttempt, delayMs: delay });
