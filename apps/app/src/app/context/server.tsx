...
export function ServerProvider(props: ParentProps & { defaultUrl: string }) {
  ...
  const readStoredList = () => {
    try {
      const raw = window.localStorage.getItem("aurowork.server.list");
  ...
  const readStoredActive = () => {
    try {
      const stored = window.localStorage.getItem("aurowork.server.active");
  ...
  createEffect(() => {
    ...
    // In hosted web deployments served by AuroWork, OpenCode
    ...
    const forceProxy =
      ...
      (import.meta.env.PROD ||
        (typeof import.meta.env?.VITE_AUROWORK_URL === "string" &&
          import.meta.env.VITE_AUROWORK_URL.trim().length > 0));
  ...
  createEffect(() => {
    ...
    try {
      window.localStorage.setItem("aurowork.server.list", JSON.stringify(list()));
      window.localStorage.setItem("aurowork.server.active", active());
    } catch {
      // ignore
    }
  });
  ...
  const readAuroworkToken = () => {
    try {
      return (window.localStorage.getItem("aurowork.server.token") ?? "").trim();
    } catch {
      return "";
    }
  };
