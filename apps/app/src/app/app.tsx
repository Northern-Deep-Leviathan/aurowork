...
function buildDefaultWorkspaceBlueprint(_preset: string): null {
  return null;
}
function normalizeWorkspaceAuroworkConfig(
  value: unknown,
  preset?: string | null,
): import("./types").WorkspaceAuroworkConfig {
  const candidate =
    value && typeof value === "object"
      ? (value as Partial<import("./types").WorkspaceAuroworkConfig>)
      : {};
...
import {
  ...
  readAuroworkBundleInviteFromSearch,
  readAuroworkConnectInviteFromSearch,
  stripAuroworkBundleInviteFromUrl,
  stripAuroworkConnectInviteFromUrl,
  createAuroworkServerClient,
  hydrateAuroworkServerSettingsFromEnv,
  normalizeAuroworkServerUrl,
  readAuroworkServerSettings,
  writeAuroworkServerSettings,
...
function defaultPresetFromTemplateBundle(bundle: SharedWorkspaceProfileBundleV1): WorkspacePreset {
  const aurowork = bundle.workspace?.aurowork;
  if (!aurowork || typeof aurowork !== "object") return "starter";
  const workspace = (aurowork as Record<string, unknown>).workspace;
...
function parseSharedBundle(value: unknown): SharedBundleV1 {
  ...
  if (type === "workspace-profile") {
    ...
    return {
      ...
      description: typeof record.description === "string" ? record.description : undefined,
      workspace: {
        ...(workspace as AuroworkWorkspaceExport),
        ...(files.length ? { files } : {}),
      },
    };
  }
...
async function fetchSharedBundle(bundleUrl: string, serverClient?: AuroworkServerClient | null): Promise<SharedBundleV1> {
  let targetUrl: URL;
  try {
...
function buildImportPayloadFromBundle(bundle: SharedBundleV1): {
  ...
  if (workspace.aurowork && typeof workspace.aurowork === "object") payload.aurowork = workspace.aurowork;
...
export default function App() {
  const envAuroworkWorkspaceId =
    typeof import.meta.env?.VITE_AUROWORK_WORKSPACE_ID === "string"
      ? import.meta.env.VITE_AUROWORK_WORKSPACE_ID.trim() || null
      : null;

  // Workspace switch tracing is noisy, so only emit in developer mode.
  // (AuroWork already has a developer mode toggle in Settings.)
  ...
  const [auroworkServerCheckedAt, setAuroworkServerCheckedAt] = createSignal<number | null>(null);
  ...
  const [auroworkAuditError, setAuroworkAuditError] = createSignal<string | null>(null);
  ...
  const [activeWorkspaceServerConfig, setActiveWorkspaceServerConfig] =
    createSignal<WorkspaceAuroworkConfig | null>(null);

  const auroworkServerBaseUrl = createMemo(() => {
    const pref = startupPreference();
    const hostInfo = auroworkServerHostInfo();
  ...
  const auroworkServerAuth = createMemo(
    () => {
      const pref = startupPreference();
      const hostInfo = auroworkServerHostInfo();
      const settingsToken = auroworkServerSettings().token?.trim() ?? "";
  ...
  const auroworkServerClient = createMemo(() => {
    const baseUrl = auroworkServerBaseUrl().trim();
    if (!baseUrl) return null;
    const auth = auroworkServerAuth();
    return createAuroworkServerClient({ baseUrl, token: auth.token, hostToken: auth.hostToken });
  });

  const devtoolsAuroworkClient = createMemo(() => auroworkServerClient());

  createEffect(() => {
    if (typeof window === "undefined") return;
    hydrateAuroworkServerSettingsFromEnv();

    const stored = readAuroworkServerSettings();
    const invite = readAuroworkConnectInviteFromSearch(window.location.search);
    const bundleInvite = readAuroworkBundleInviteFromSearch(window.location.search);

    if (!invite) {
      setAuroworkServerSettings(stored);
    } else {
      const merged: AuroworkServerSettings = {
        ...stored,
    ...
    const cleaned = stripAuroworkBundleInviteFromUrl(cleanedConnect);
    ...
    const applyAndPersistFontZoom = (value: number) => {
      ...
      try {
        ...
        void applyWebviewZoom(webview, next)
          .then(() => {
            document.documentElement.style.removeProperty("--aurowork-font-size");
          })
          .catch(() => {
  ...
  createEffect(() => {
    ...
    if (pref === "local") {
      setAuroworkServerUrl(hostUrl);
      return;
    }
    if (pref === "server") {
      setAuroworkServerUrl(settingsUrl);
      return;
    }
    setAuroworkServerUrl(hostUrl || settingsUrl);
  });

  const checkAuroworkServer = async (url: string, token?: string, hostToken?: string) => {
    const client = createAuroworkServerClient({ baseUrl: url, token, hostToken });
    try {
      await client.health();
    } catch (error) {
      if (error instanceof AuroworkServerError && (error.status === 401 || error.status === 403)) {
        return { status: "limited" as AuroworkServerStatus, capabilities: null };
      }
      return { status: "disconnected" as AuroworkServerStatus, capabilities: null };
    }
    ...
    try {
      ...
      return { status: "connected" as AuroworkServerStatus, capabilities: caps };
    } catch (error) {
      if (error instanceof AuroworkServerError && (error.status === 401 || error.status === 403)) {
        return { status: "limited" as AuroworkServerStatus, capabilities: null };
      }
  ...
  createEffect(() => {
    ...
    const url = auroworkServerBaseUrl().trim();
    const auth = auroworkServerAuth();
    const token = auth.token;
    const hostToken = auth.hostToken;

    if (!url) {
      setAuroworkServerStatus("disconnected");
      setAuroworkServerCapabilities(null);
    ...
    const run = async () => {
      ...
      busy = true;
      try {
        const result = await checkAuroworkServer(url, token, hostToken);
        if (!active) return;
        setAuroworkServerStatus(result.status);
  ...
  createEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (active) setAuroworkServerHostInfo(info);
      } catch {
        if (active) setAuroworkServerHostInfo(null);
      }
    };
  ...
  createEffect(() => {
    ...
    const current = auroworkServerSettings();
  ...
  createEffect(() => {
    ...
    if (!documentVisible()) return;
    if (!developerMode()) {
      setAuroworkServerDiagnostics(null);
      return;
    }

    const client = auroworkServerClient();
    if (!client || auroworkServerStatus() === "disconnected") {
      setAuroworkServerDiagnostics(null);
      return;
    }
    ...
    const run = async () => {
      ...
      try {
        ...
        if (active) setAuroworkServerDiagnostics(status);
      } catch {
        if (active) setAuroworkServerDiagnostics(null);
      } finally {
        busy = false;
      }
    };
  ...
  const SESSION_BY_WORKSPACE_KEY = "aurowork.workspace-last-session.v1";
  ...
  async function sendPrompt(draft?: ComposerDraft) {
    ...
    debugFileLog("prompt", "sendPrompt", { textLength: fallbackText.length, hasExplicitDraft });
  ...
  async function abortSession(sessionID?: string) {
    ...
    debugFileLog("prompt", "abortSession", { sessionID: id });
  ...
  // as the visibility boundary. AuroWork mirrors that behavior by filtering the
  ...
  function focusSessionPromptSoon() {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        window.dispatchEvent(new CustomEvent("aurowork:focusPrompt"));
      });
    });
  }
  ...
  const [scheduledJobs, setScheduledJobs] = createSignal<unknown[]>([]);
  ...
  const extensionsStore = createExtensionsStore({
    ...
    onNotionSkillInstalled: () => {
      ...
      try {
        window.localStorage.setItem("aurowork.notionSkillInstalled", "1");
      } catch {
        // ignore
      }
      if (notionIsActive()) {
  ...
  const workspaceStore = createWorkspaceStore({
    ...
    auroworkServerClient,
    auroworkServerStatus,
    ensureLocalAuroworkServerClient,
    runtimeWorkspaceId,
    onEngineStable: () => {},
    engineRuntime,
    developerMode,
  ...
  createEffect(() => {
    ...
    // definitions themselves. Global AuroWork server settings are intentionally excluded so that
  ...
  createEffect(() => {
    ...
    if (!client || auroworkServerStatus() !== "connected") {
  ...
  const resolveSharedBundleWorkerTarget = () => {
    ...
    const localHostUrl = normalizeAuroworkServerUrl(hostInfo?.baseUrl ?? "") ?? "";
    const localToken = hostInfo?.clientToken?.trim() ?? "";
    const serverHostUrl = normalizeAuroworkServerUrl(settings.urlOverride ?? "") ?? "";
  ...
  const waitForSharedBundleImportTarget = async (timeoutMs = 20_000, target?: SharedBundleImportTarget) => {
    ...
    throw new Error("AuroWork worker is not ready yet.");
  };
  ...
  const importSharedBundlePayload = async (bundle: SharedBundleV1, target?: SharedBundleImportTarget) => {
    ...
    if (importedSkillsCount > 0) {
      ...
      console.log(`[aurowork] imported ${importedSkillsCount} skills from share bundle`);
    }
  };
  ...
  const importSharedBundleIntoActiveWorker = async (
    ...
    try {
      const bundle = bundleOverride ?? (await fetchSharedBundle(request.bundleUrl, auroworkServerClient()));
  ...
  const processSharedBundleInvite = async (request: SharedBundleDeepLink) => {
    const bundle = await fetchSharedBundle(request.bundleUrl, auroworkServerClient());
    ...
    if (request.intent === "import_current") {
      ...
      if (!client || !connected || !hasTargetHint) {
        if (!sharedBundleNoticeShown()) {
          ...
          setError("Share link detected. Connect to a writable AuroWork worker to import this bundle.");
        }
      ...
      if (!target.hostUrl.trim() || !target.token.trim()) {
        if (!sharedBundleNoticeShown()) {
          setSharedBundleNoticeShown(true);
          setError("Share link detected. Configure an AuroWork host and token to create a new worker.");
        }
  ...
  const auroworkServerCanWriteSkills = createMemo(
    () =>
      auroworkServerReady() &&
      auroworkServerWorkspaceReady() &&
      (resolvedAuroworkCapabilities()?.skills?.write ?? false),
  );
  const auroworkServerCanWritePlugins = createMemo(
    () =>
      auroworkServerReady() &&
      auroworkServerWorkspaceReady() &&
      (resolvedAuroworkCapabilities()?.plugins?.write ?? false),
  );
  const auroworkServerCanReadConfig = createMemo(
    () =>
      auroworkServerReady() &&
      auroworkServerWorkspaceReady() &&
      (resolvedAuroworkCapabilities()?.config?.read ?? false),
  );
  const auroworkServerCanWriteConfig = createMemo(
    () =>
      auroworkServerReady() &&
      auroworkServerWorkspaceReady() &&
      (resolvedAuroworkCapabilities()?.config?.write ?? false),
  );
  const devtoolsCapabilities = createMemo(() => auroworkServerCapabilities());

  function updateAuroworkServerSettings(next: AuroworkServerSettings) {
    const stored = writeAuroworkServerSettings(next);
    setAuroworkServerSettings(stored);
  }
  ...
  const importSharedBundleIntoExistingWorkspace = async (workspaceId: string) => {
    ...
    try {
      setView("dashboard");
      setTab(choice.bundle.type === "workspace-profile" ? "skills" : "skills");
  ...
  const testAuroworkServerConnection = async (next: AuroworkServerSettings) => {
    const derived = normalizeAuroworkServerUrl(next.urlOverride ?? "");
    if (!derived) {
      setAuroworkServerStatus("disconnected");
      setAuroworkServerCapabilities(null);
      setAuroworkServerCheckedAt(Date.now());
    ...
    const result = await checkAuroworkServer(derived, next.token, auroworkServerAuth().hostToken);
    setAuroworkServerStatus(result.status);
    setAuroworkServerCapabilities(result.capabilities);
    setAuroworkServerCheckedAt(Date.now());
  ...
  const reconnectAuroworkServer = async () => {
    if (auroworkReconnectBusy()) return false;
    setAuroworkReconnectBusy(true);
    try {
      let hostInfo = auroworkServerHostInfo();
      if (isTauriRuntime()) {
        try {
          hostInfo = await auroworkServerInfo();
          setAuroworkServerHostInfo(hostInfo);
        } catch {
          hostInfo = null;
          setAuroworkServerHostInfo(null);
        }
      }
      ...
      if (hostInfo?.clientToken?.trim() && startupPreference() !== "server") {
        ...
        if ((settings.token?.trim() ?? "") !== liveToken) {
          updateAuroworkServerSettings({ ...settings, token: liveToken });
        }
      }
      ...
      const result = await checkAuroworkServer(url, auth.token, auth.hostToken);
      setAuroworkServerStatus(result.status);
      setAuroworkServerCapabilities(result.capabilities);
      setAuroworkServerCheckedAt(Date.now());
      return result.status === "connected" || result.status === "limited";
    } finally {
      setAuroworkReconnectBusy(false);
    }
  };

  async function ensureLocalAuroworkServerClient(): Promise<AuroworkServerClient | null> {
    let hostInfo = auroworkServerHostInfo();
    if (hostInfo?.baseUrl?.trim() && hostInfo.clientToken?.trim()) {
      const existing = createAuroworkServerClient({
    ...
    return createAuroworkServerClient({
      baseUrl,
      token,
  ...
  const {
    ...
    dockerCleanupBusy,
    dockerCleanupResult,
    cleanupAuroworkDockerContainers,
    updateAutoCheck,
    setUpdateAutoCheck,
    updateAutoDownload,
  ...
  const resetAppConfigDefaults = async () => {
    try {
      ...
      clearAuroworkServerSettings();
      setAuroworkServerSettings(readAuroworkServerSettings());
      ...
      return { ok: true, message: "Reset app config defaults. Restart AuroWork if any stale settings remain." };
    } catch (error) {
  ...
  onMount(() => {
    // OpenCode hot reload drives freshness now; AuroWork no longer listens for
  ...
  const deleteScheduledJob = async (_name: string) => {
    // scheduler feature removed
    return;
};
  ...
  const refreshActiveWorkspaceServerConfig = async (
  ...
  ): Promise<WorkspaceAuroworkConfig | null> => {
    const client = auroworkServerClient();
    ...
    const normalized = normalizeWorkspaceAuroworkConfig(
      config.aurowork,
      workspace.preset,
    );
  ...
  createEffect(() => {
    ...
    const workspaceId = runtimeWorkspaceId();
    const capabilities = resolvedAuroworkCapabilities();
    const canReadConfig =
      auroworkServerStatus() === "connected" &&
      Boolean(auroworkClient && workspaceId && capabilities?.config?.read);

    if (!canReadConfig || !auroworkClient || !workspaceId) {
    ...
    const loadWorkspaceConfig = async () => {
      try {
        const config = await auroworkClient.getConfig(workspaceId);
        if (cancelled) return;
  ...
  async function connectNotion() {
    ...
    const canUseAuroworkServer =
      ...
      auroworkClient &&
      auroworkWorkspaceId &&
      auroworkCapabilities?.mcp?.write;

    if (!canUseAuroworkServer && !isTauriRuntime()) {
    ...
    try {
      if (canUseAuroworkServer) {
        await auroworkClient.addMcp(auroworkWorkspaceId, {
          name: "notion",
          config: {
            type: "remote",
      ...
      setNotionStatusDetail(t("mcp.connecting", currentLocale()));
      try {
        window.localStorage.setItem("aurowork.notionStatus", "connecting");
        window.localStorage.setItem("aurowork.notionStatusDetail", t("mcp.connecting", currentLocale()));
        window.localStorage.setItem("aurowork.notionSkillInstalled", "0");
      } catch {
        // ignore
      }
    } catch (e) {
  ...
  async function connectMcp(entry: (typeof MCP_QUICK_CONNECT)[number]) {
    ...
    const auroworkClient = auroworkServerClient();
    let auroworkWorkspaceId = runtimeWorkspaceId();
    const auroworkCapabilities = resolvedAuroworkCapabilities();
    if (!auroworkWorkspaceId && auroworkClient && auroworkServerStatus() === "connected") {
      try {
        const response = await auroworkClient.listWorkspaces();
        const match = response.items?.[0];
        if (match?.id) {
          auroworkWorkspaceId = match.id;
    ...
    if (isRemoteWorkspace && !canUseAuroworkServer) {
      setMcpStatus("AuroWork server unavailable. MCP config is read-only.");
      finishPerf(developerMode(), "mcp.connect", "blocked", startedAt, {
        reason: "aurowork-server-unavailable",
      });
      return;
    }
    ...
    if (!activeClient) {
      const auroworkBaseUrl = auroworkServerBaseUrl().trim();
      const auth = auroworkServerAuth();
      if (auroworkBaseUrl && auth.token) {
        const opencodeUrl = `${auroworkBaseUrl.replace(/\/+$/, "")}/opencode`;
        activeClient = createClient(opencodeUrl, undefined, { token: auth.token, mode: "aurowork" });
  ...
  async function logoutMcpAuth(name: string) {
    ...
    if (isRemoteWorkspace && !canUseAuroworkServer) {
      setMcpStatus("AuroWork server unavailable. MCP auth is read-only.");
      return;
    }
    ...
    try {
      if (canUseAuroworkServer && auroworkClient && auroworkWorkspaceId) {
        await auroworkClient.logoutMcpAuth(auroworkWorkspaceId, safeName);
      } else {
        try {
  ...
  async function removeMcp(name: string) {
    try {
      ...
      const canUseAuroworkServer =
        ...
        auroworkClient &&
        auroworkWorkspaceId &&
        resolvedAuroworkCapabilities()?.mcp?.write;

      if (canUseAuroworkServer && auroworkClient && auroworkWorkspaceId) {
        await auroworkClient.removeMcp(auroworkWorkspaceId, name);
      } else {
        const projectDir = workspaceProjectDir().trim();
        if (!projectDir) {
          setMcpStatus(t("mcp.pick_workspace_first", currentLocale()));
  ...
  async function createSessionAndOpen() {
    ...
    const startedAt = perfNow();
    const runId = (() => {
      const key = "__aurowork_create_session_run__";
  ...
  onMount(async () => {
    ...
    if (typeof window !== "undefined") {
      try {
        ...
        if (!isTauriRuntime()) {
          const storedBaseUrl = window.localStorage.getItem("aurowork.baseUrl");
          if (storedBaseUrl) {
        ...
        const storedEngineRuntime = window.localStorage.getItem(
          "aurowork.engineRuntime"
        );
        if (storedEngineRuntime === "direct" || storedEngineRuntime === "aurowork-orchestrator") {
        ...
        const storedNotionStatus = window.localStorage.getItem("aurowork.notionStatus");
        if (
        ...
        const storedNotionDetail = window.localStorage.getItem("aurowork.notionStatusDetail");
  ...
  createEffect(() => {
    const auroworkClient = auroworkServerClient();
    const auroworkWorkspaceId = runtimeWorkspaceId();
    const canReadConfig = auroworkServerCanReadConfig();

    if (!auroworkClient || !auroworkWorkspaceId || !canReadConfig) {
  ...
  const persistAuthorizedFolders = async (nextFolders: string[]) => {
    const auroworkClient = auroworkServerClient();
    const auroworkWorkspaceId = runtimeWorkspaceId();
    if (!auroworkClient || !auroworkWorkspaceId || !auroworkServerCanWriteConfig()) {
      setAuthorizedFoldersError(
        "A writable AuroWork server workspace is required to update authorized folders.",
      );
      return false;
    }
    ...
    try {
      const currentConfig = await auroworkClient.getConfig(auroworkWorkspaceId);
  ...
  createEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("aurowork.baseUrl", baseUrl());
    } catch {
      // ignore
    }
  });
  ...
  createEffect(() => {
    ...
    try {
      window.localStorage.setItem("aurowork.projectDir", workspaceProjectDir());
    } catch {
      // ignore
    }
  });
  ...
  createEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("aurowork.engineSource", engineSource());
    } catch {
      // ignore
    }
  });
  ...
  createEffect(() => {
    ...
    try {
      ...
      if (value) {
        window.localStorage.setItem("aurowork.engineCustomBinPath", value);
      } else {
        window.localStorage.removeItem("aurowork.engineCustomBinPath");
      }
    } catch {
      // ignore
    }
  ...
  createEffect(() => {
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("aurowork.engineRuntime", engineRuntime());
    } catch {
      // ignore
    }
  });
  ...
  const headerConnectedVersion = createMemo(() => {
    ...
    const auroworkVersion =
      appVersion()?.trim() ||
      auroworkServerDiagnostics()?.version?.trim() ||
      "";
    if (!auroworkVersion) {
      return fallbackVersion || null;
    }

    const normalizedVersion = auroworkVersion.startsWith("v")
      ? auroworkVersion
      : `v${auroworkVersion}`;
    return `AuroWork ${normalizedVersion}`;
  });
  ...
  const workspaceSwitchWorkspace = createMemo(() => {
    ...
    if (switchingId) {
      return workspaceStore.workspaces().find((ws) => ws.id === switchingId) ?? selectedWorkspaceDisplay();
    }
  ...
  const onboardingProps = () => ({
    ...
    auroworkHostUrl: auroworkServerSettings().urlOverride ?? "",
    auroworkToken: auroworkServerSettings().token ?? "",
    ...
    onAuroworkHostUrlChange: (value: string) =>
      updateAuroworkServerSettings({
        ...auroworkServerSettings(),
        urlOverride: value,
      }),
    onAuroworkTokenChange: (value: string) =>
      updateAuroworkServerSettings({
        ...auroworkServerSettings(),
        token: value,
      }),
  ...
  const dashboardProps = () => {
    ...
    return {
      ...
      error: error(),
      auroworkServerStatus: auroworkStatus,
      auroworkServerUrl: auroworkServerUrl(),
      auroworkServerClient: auroworkServerClient(),
      auroworkReconnectBusy: auroworkReconnectBusy(),
      ...
      auroworkAuditEntries: auroworkAuditEntries(),
      auroworkAuditStatus: auroworkAuditStatus(),
      auroworkAuditError: auroworkAuditError(),
      ...
      authorizedFoldersAvailable: auroworkServerCanReadConfig(),
      authorizedFoldersEditable: auroworkServerCanWriteConfig(),
      authorizedFoldersHint: !auroworkServerReady()
        ? "AuroWork server is disconnected."
        : !auroworkServerWorkspaceReady()
          ? "No active server workspace is selected."
          : !auroworkServerCanReadConfig()
            ? "AuroWork server config access is unavailable for this workspace."
            : !auroworkServerCanWriteConfig()
              ? "AuroWork server is connected read-only for workspace config."
              : null,
      addAuthorizedFolder,
