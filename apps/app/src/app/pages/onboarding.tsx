...
export type OnboardingViewProps = {
  ...
  onAuroworkHostUrlChange: (value: string) => void;
  onAuroworkTokenChange: (value: string) => void;
  ...
  return (
    <Switch>
      <Match when={props.onboardingStep === "connecting" || props.onboardingStep === "bootstrap"}>
        <div class="min-h-screen flex flex-col items-center justify-center bg-dls-surface text-dls-text p-6 relative overflow-hidden">
          <div class="absolute inset-0 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-dls-hover via-dls-surface to-dls-surface opacity-50" />
          <div class="z-10 w-full max-w-lg rounded-[28px] border border-dls-border bg-dls-surface/95 p-8 shadow-[var(--dls-shell-shadow)]">
            <div class="flex flex-col items-center gap-6 text-center">
              <div class="flex h-16 w-16 items-center justify-center rounded-[22px] border border-dls-border bg-dls-sidebar shadow-[var(--dls-card-shadow)]">
                <Show when={props.onboardingStep === "bootstrap"} fallback={<AuroWorkLogo size={36} />}>
                  <HardDrive size={24} class="text-dls-secondary" />
                </Show>
              </div>
              <div>
                ...
                <h2 class="mt-3 text-2xl font-semibold tracking-tight text-dls-text">
              ...
              <div class="w-full rounded-[22px] border border-dls-border bg-dls-sidebar px-5 py-4 text-left">
                ...
                <div class="mt-2 text-xs leading-6 text-dls-secondary">
                  {props.onboardingStep === "bootstrap"
                    ...
                    : "Checking your local AuroWork services and reconnecting to the selected workspace."}
                </div>
              </div>
              ...
              <Show when={connectingFallbackVisible()}>
                <div class="flex flex-col items-center gap-3">
                  ...
                  <div class="text-xs text-dls-secondary">Open Advanced settings to check local host configuration.</div>
                </div>
              </Show>
            </div>
          </div>
      ...
      <Match when={props.onboardingStep === "local"}>
        <div class="min-h-screen flex flex-col items-center justify-center bg-dls-surface text-dls-text p-6 relative">
          ...
          <div class="max-w-lg w-full z-10 space-y-6">
            ...
            <div class="space-y-4">
              <div class="bg-dls-hover/40 border border-dls-border rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3">
                <div>
                  <div class="text-xs font-semibold text-dls-secondary uppercase tracking-wider">{translate("onboarding.theme_label")}</div>
                  <div class="text-sm text-dls-text">{translate("onboarding.theme_current").replace("{mode}", props.themeMode)}</div>
                </div>
                <div class="flex flex-wrap gap-2">
                  <button
                    class={`text-xs px-3 py-1 rounded-full border transition-colors ${
                      props.themeMode === "system"
                        ? "bg-dls-text/10 text-dls-text border-dls-border/30"
              ...
              <div class="rounded-2xl border border-dls-border bg-dls-surface/50 px-4 py-3">
                <div class="flex items-center justify-between gap-4">
                  <div class="min-w-0">
                    <div class="text-xs font-semibold text-dls-secondary uppercase tracking-wider">Import</div>
                    <div class="mt-1 text-sm text-dls-text">Use an existing workspace config.</div>
                    <div class="text-xs text-dls-secondary">Imports `.opencode` and `opencode.json` only.</div>
                  </div>
                  <Button
                    variant="secondary"
              ...
              <div class="rounded-2xl border border-dls-border bg-dls-surface/50 px-4 py-3">
                <div class="flex items-center justify-between gap-4">
                  <div class="min-w-0">
                    <div class="text-xs font-semibold text-dls-secondary uppercase tracking-wider">{translate("onboarding.access_label")}</div>
                    <div class="mt-1 text-sm text-dls-text">
                    ...
                    <div class="text-xs text-dls-secondary">{translate("onboarding.manage_access_hint")}</div>
                  </div>
                  <div class="text-xs text-dls-secondary font-mono truncate max-w-[9rem]">
            ...
            <details class="rounded-2xl border border-dls-border bg-dls-surface/60 px-4 py-3">
              <summary class="flex items-center justify-between cursor-pointer text-xs text-dls-secondary">
                {translate("onboarding.advanced_settings")}
                <ChevronDown size={14} class="text-dls-secondary" />
              </summary>
              <div class="pt-3 space-y-3">
                <div class="text-xs text-dls-secondary">{translate("onboarding.manage_access_hint")}</div>

                <div class="flex items-center justify-between gap-3 rounded-xl border border-dls-border bg-dls-surface/40 px-3 py-2">
                  <div class="text-xs text-dls-secondary">{translate("onboarding.open_settings_hint")}</div>
                  <Button
                    variant="outline"
                ...
                <div class="space-y-3">
                  ...
                  <div class="text-xs text-dls-secondary">{engineStatusLabel()}</div>

                  <Show when={props.authorizedDirs.length}>
                    <div class="space-y-2">
                      <For each={props.authorizedDirs}>
                        {(dir, idx) => (
                          <div class="flex items-center justify-between gap-3 rounded-xl bg-dls-surface/20 border border-dls-border px-3 py-2">
                            <div class="min-w-0 text-xs font-mono text-dls-secondary truncate">{dir}</div>
                            <Button
                              variant="ghost"
                              class="!p-2 rounded-lg text-xs text-dls-secondary hover:text-dls-text"
                ...
                <Show when={isTauriRuntime() && props.developerMode}>
                  <div class="rounded-2xl bg-dls-hover/40 border border-dls-border p-4">
                    <div class="flex items-start justify-between gap-4">
                      <div class="min-w-0">
                        <div class="text-sm font-medium text-dls-text">{translate("onboarding.cli_label")}</div>
                          ...
                          <Show when={props.engineDoctorFound != null} fallback={<span>{translate("onboarding.cli_checking")}</span>}>
                            <Show when={props.engineDoctorFound} fallback={<span>{translate("onboarding.cli_not_found_hint")}</span>}>
                              ...
                              <Show when={props.engineDoctorResolvedPath}>
                                <span class="text-dls-secondary"> · </span>
                                <span class="font-mono text-dls-secondary truncate">{props.engineDoctorResolvedPath}</span>
                              </Show>
                            </Show>
                          </Show>
                        </div>
                    ...
                    <Show when={props.engineDoctorFound === false}>
                      <div class="mt-4 space-y-2">
                        ...
                        <Show when={isWindowsPlatform()}>
                          <div class="text-xs text-dls-secondary space-y-1 font-mono">
                    ...
                    <Show when={props.engineInstallLogs}>
                      <pre class="mt-4 max-h-48 overflow-auto rounded-xl bg-dls-surface/50 border border-dls-border p-3 text-xs text-dls-secondary whitespace-pre-wrap">
                        {props.engineInstallLogs}
                      </pre>
                    </Show>

                    <Show when={props.engineDoctorCheckedAt != null}>
                      <div class="mt-3 text-[11px] text-dls-secondary">
                ...
                <Show when={engineDoctorAvailable()}>
                  <div class="text-xs text-dls-secondary">{translate("onboarding.ready_message")}</div>
                </Show>
                ...
                >
                  <div class="rounded-xl bg-dls-surface/40 border border-dls-border p-3 space-y-3 text-xs text-dls-secondary">
                    <Show when={props.engineDoctorResolvedPath}>
                      <div>
                        <div class="text-[11px] text-dls-secondary">{translate("onboarding.resolved_path")}</div>
                        <div class="font-mono break-all">{props.engineDoctorResolvedPath}</div>
                      </div>
                    </Show>
                    <Show when={props.engineDoctorVersion}>
                      <div>
                        <div class="text-[11px] text-dls-secondary">{translate("onboarding.version")}</div>
                        <div class="font-mono">{props.engineDoctorVersion}</div>
                      </div>
                    </Show>
                    <Show when={props.engineDoctorNotes.length}>
                      <div>
                        <div class="text-[11px] text-dls-secondary">{translate("onboarding.search_notes")}</div>
                        <pre class="whitespace-pre-wrap break-words text-xs text-dls-secondary">
                    ...
                    <Show when={serveHelpOutput()}>
                      <div>
                        <div class="text-[11px] text-dls-secondary">{translate("onboarding.serve_help")}</div>
                        <pre class="whitespace-pre-wrap break-words text-xs text-dls-secondary">{serveHelpOutput()}</pre>
                      </div>
                    </Show>
                  </div>
                </Show>
          ...
          <div class="max-w-xl w-full z-10 space-y-12">
            <div class="text-center space-y-4">
              <div class="flex items-center justify-center gap-3 mb-6">
                ...
                <h1 class="text-3xl font-bold tracking-tight text-dls-text">AuroWork</h1>
              </div>
              <h2 class="text-xl text-dls-secondary">{translate("onboarding.welcome_title")}</h2>
            </div>

            <div class="space-y-4">
              <button
                onClick={() => props.onSelectStartup("local")}
                class="group w-full relative bg-dls-hover hover:bg-dls-active border border-dls-border hover:border-dls-border p-6 md:p-8 rounded-3xl text-left transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-6/10 hover:-translate-y-0.5 flex items-start gap-6"
              >
                ...
                <div>
                  <h3 class="text-xl font-medium text-dls-text mb-2">{translate("onboarding.run_local")}</h3>
              ...
              <Show when={props.engineRunning && props.engineBaseUrl}>
                <div class="rounded-2xl bg-dls-hover/40 border border-dls-border p-5 flex items-center justify-between">
                  <div>
                    <div class="text-sm text-dls-text font-medium">{translate("onboarding.engine_running")}</div>
                    <div class="text-xs text-dls-secondary">{translate("onboarding.attach_description")}</div>
                    <Show when={props.developerMode}>
                      <div class="text-xs text-dls-secondary font-mono truncate max-w-[14rem] md:max-w-[22rem]">
                        {props.engineBaseUrl}
                      </div>
                    </Show>
                  </div>
              ...
              <div class="flex items-center gap-2 px-2 py-1">
                <button
                  ...
                  class="flex items-center gap-2 text-xs text-dls-secondary hover:text-dls-secondary transition-colors group"
                >
                  <div
                    class={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${
                      props.rememberStartupChoice
                        ? "bg-indigo-7 border-indigo-7 text-dls-text"
                        : "border-dls-border bg-transparent group-hover:border-dls-border"
                    }`}
                  >
              ...
              <Show when={props.developerMode}>
                <div class="text-center text-xs text-dls-secondary">{props.localHostLabel}</div>
              </Show>
            </div>
          </div>
        </div>
