...
// Stub functions for removed workspace-blueprints module
function defaultBlueprintCopyForPreset(_preset: string): { title: string; body: string } {
...
export type SessionViewProps = {
  ...
  activeWorkspaceConfig: WorkspaceAuroworkConfig | null;
...
function describePermissionRequest(permission: PendingPermission | null) {
  ...
  if (!permission) {
    return {
      title: tr("session.permission_required"),
      message: tr("session.permission_description"),
      permissionLabel: "",
      scopeLabel: tr("session.scope_label"),
      scopeValue: "",
      isDoomLoop: false,
  ...
  if (permission.permission === "doom_loop") {
    const tool =
      permission.metadata && typeof permission.metadata === "object" && typeof permission.metadata.tool === "string"
        ...
        : null;
    return {
      title: tr("session.doom_loop_title"),
      message: tr("session.doom_loop_message"),
      permissionLabel: tr("session.doom_loop_label"),
      scopeLabel: tool ? tr("session.doom_loop_scope_tool") : tr("session.doom_loop_scope_repeated"),
      scopeValue: tool ?? (patterns.length ? patterns.join(", ") : tr("session.doom_loop_repeated_call")),
      isDoomLoop: true,
      note: tr("session.doom_loop_note"),
    };
  }
...
export default function SessionView(props: SessionViewProps) {
  ...
  const openFeedback = () => {
    const resolved = buildFeedbackUrl({
      ...
      deployment: getAuroWorkDeployment(),
      appVersion: props.appVersion,
      auroworkServerVersion: props.auroworkServerDiagnostics?.version ?? null,
      opencodeVersion:
  ...
  const activeSearchPositionLabel = createMemo(() => {
    const hits = searchHits();
    if (!hits.length) return translate("session.no_matches");
    const size = hits.length;
    const raw = activeSearchHitIndex();
    const index = ((raw % size) + size) % size;
  ...
  const resolveLocalFileCandidates = async (file: string) => {
    ...
    if (normalized.startsWith(".opencode/aurowork/outbox/")) {
      return candidates;
    }

    if (normalized.startsWith("aurowork/outbox/")) {
      const suffix = normalized.slice("aurowork/outbox/".length);
      if (suffix) {
        pushCandidate(
          await join(root, ".opencode", "aurowork", "outbox", suffix),
        );
      }
      return candidates;
    }
    ...
    if (!normalized.startsWith(".opencode/")) {
      pushCandidate(
        await join(root, ".opencode", "aurowork", "outbox", normalized),
      );
    }
  ...
  const runLocalFileAction = async (
    ...
    return {
      ok: false as const,
      reason: `${lastError instanceof Error ? lastError.message : translate("session.file_open_failed")}${suffix}`,
    };
  };
  ...
  const revealWorkspaceInFinder = async (workspaceId: string) => {
    ...
    if (!target) {
      setToastMessage(translate("session.workspace_path_unavailable"));
      return;
    }
    if (!isTauriRuntime()) {
      setToastMessage(translate("session.reveal_desktop_only"));
      return;
    }
    try {
      ...
      const message =
        error instanceof Error ? error.message : translate("session.reveal_failed");
      setToastMessage(message);
    }
  };
  const todoLabel = createMemo(() => {
    ...
    if (!total) return "";
    return `${todoCompletedCount()} ${translate("session.todo_out_of")} ${total} ${translate("session.todo_tasks_completed")}`;
  });
  ...
  const attachmentsDisabledReason = createMemo(() => {
    ...
    if (props.auroworkServerStatus === "limited") {
      return translate("session.add_server_token_to_attach");
    }
    return translate("session.connect_server_to_attach");
  });
  ...
  const handleWorkingFileClick = async (file: string) => {
    ...
    try {
      ...
      if (!result.ok && result.reason === "missing-root") {
        setToastMessage(translate("session.pick_workspace_to_open"));
        return;
      }
      if (!result.ok) {
      ...
      const message =
        error instanceof Error ? error.message : translate("session.unable_to_open_file");
      setToastMessage(message);
    }
  };
  ...
  const loadAgentOptions = async (force = false) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : translate("session.load_agents_failed");
  ...
  const computeStatusFromPart = (part: Part | null) => {
    ...
    if (part.type === "tool") {
      ...
      switch (tool) {
        case "task":
          return translate("session.phase_delegating");
        case "todowrite":
        case "todoread":
          return translate("session.phase_planning");
        case "read":
          return translate("session.phase_gathering_context");
        case "list":
        case "grep":
        case "glob":
          return translate("session.phase_searching_codebase");
        case "webfetch":
          return translate("session.phase_searching_web");
        case "edit":
        case "write":
        case "apply_patch":
          return translate("session.phase_writing_file");
        case "bash":
          return translate("session.phase_running_shell");
        default:
          return translate("session.phase_working");
      }
    }
    if (part.type === "reasoning") {
      return translate("session.phase_thinking");
    }
    if (part.type === "text") {
      return null;
    }
  ...
  const thinkingStatus = createMemo(() => {
    ...
    if (runPhase() === "thinking") return translate("session.phase_thinking");
    return null;
  });
  ...
  const cancelRun = async () => {
    ...
    if (!props.selectedSessionId) {
      setToastMessage(translate("session.no_session_selected"));
      return;
    }
    ...
    try {
      ...
      setToastMessage(translate("session.stopped"));
    } catch (error) {
      const message = error instanceof Error ? error.message : translate("session.stop_failed");
      setToastMessage(message);
    } finally {
      setAbortBusy(false);
    }
  ...
  const compactSessionHistory = async () => {
    ...
    setToastMessage(translate("session.compacting"));
    try {
  ...
  createEffect(() => {
    ...
    if (count > prev && prev > 0) {
      ...
      triggerFlyout(lastMsg ?? null, "sidebar-progress", translate("session.flyout_new_task"), "check");
    }
  ...
  createEffect(() => {
    ...
    if (count > prev && prev > 0) {
      ...
      triggerFlyout(
        lastMsg ?? null,
        "sidebar-context",
        translate("session.flyout_file_modified"),
        "folder",
      );
    }
  ...
  const confirmDeleteSession = async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : props.safeStringify(error);
      setToastMessage(message || translate("session.delete_session_failed"));
    } finally {
  ...
  const handleProviderAuthSelect = async (
    ...
    if (providerAuthActionBusy()) {
      throw new Error(translate("session.provider_auth_busy"));
    }
  ...
  const handleProviderAuthOAuth = async (
    ...
    try {
      ...
      if (result.connected) {
        setToastMessage(result.message || translate("session.provider_connected"));
  ...
  const handleProviderAuthApiKey = async (
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : translate("session.api_key_save_failed");
      setToastMessage(message);
    } finally {
      setProviderAuthActionBusy(false);
    }
  ...
  const [shareLocalAuroworkWorkspaceId, setShareLocalAuroworkWorkspaceId] =
  ...
  createEffect(() => {
    ...
    if (
      ...
      setShareLocalAuroworkWorkspaceId(null);
      return;
    }
    ...
    void (async () => {
      try {
        ...
        setShareLocalAuroworkWorkspaceId(match?.id ?? null);
      } catch {
        if (!cancelled) setShareLocalAuroworkWorkspaceId(null);
      }
    })();
  ...
  const shareNote = createMemo(() => {
    ...
    if (
      ...
      return translate("session.share_direct_runtime_note");
    }
    return null;
  });
    ...
    try {
      ...
      setShareWorkspaceProfileError(
        error instanceof Error
          ? error.message
          : translate("session.share_publish_failed"),
      );
    } finally {
      setShareWorkspaceProfileBusy(false);
    }
  ...
  const uploadInboxFiles = async (
    ...
    if (!client || !workspaceId) {
      if (notify) {
        setToastMessage(
          translate("session.upload_connect_server"),
        );
      }
      return [];
    }
    ...
    if (notify) {
      setToastMessage(`${translate("session.upload_uploading")} ${label} ${translate("session.upload_to_shared_folder")}`);
    }
    ...
    try {
      ...
      if (notify) {
        ...
        setToastMessage(
          summary
            ? `${translate("session.upload_success_prefix")} ${summary}`
            : translate("session.upload_success"),
        );
      }
      return uploaded;
    } catch (error) {
      if (notify) {
        const message =
          error instanceof Error
            ? error.message
            : translate("session.upload_failed");
        setToastMessage(message);
      }
      return [];
    }
  ...
  const commandPaletteRootItems = createMemo<CommandPaletteItem[]>(() => {
    const items: CommandPaletteItem[] = [
      {
        id: "new-session",
        title: translate("session.cmd_create_session"),
        detail: translate("session.cmd_create_session_detail"),
        meta: translate("session.cmd_create_meta"),
        action: () => {
          closeCommandPalette();
          void Promise.resolve(props.createSessionAndOpen())
            ...
            .catch((error) => {
              const message =
                error instanceof Error
                  ? error.message
                  : translate("session.cmd_create_session_failed");
              setToastMessage(message);
            });
        },
      },
      {
        id: "rename-session",
        title: translate("session.cmd_rename_session"),
        detail:
          selectedSessionTitle().trim() ||
          translate("session.cmd_rename_session_detail"),
        meta: translate("session.cmd_rename_meta"),
        action: () => {
          closeCommandPalette();
      ...
      {
        id: "sessions",
        title: translate("session.cmd_search_sessions"),
        detail: `${totalSessionCount().toLocaleString()} ${translate("session.cmd_sessions_available")}`,
        meta: translate("session.cmd_jump_meta"),
        action: () => {
      ...
      {
        id: "model",
        title: translate("session.cmd_change_model"),
        detail: `${props.selectedSessionModelLabel || translate("session.model")} · ${props.modelVariantLabel}`,
        meta: translate("session.cmd_open_meta"),
        action: () => {
          closeCommandPalette();
      ...
      {
        id: "provider",
        title: translate("session.cmd_connect_provider"),
        detail: translate("session.cmd_connect_provider_detail"),
        meta: translate("session.cmd_open_meta"),
        action: () => {
          closeCommandPalette();
          void props
            ...
            .catch((error) => {
              const message =
                error instanceof Error
                  ? error.message
                  : translate("session.cmd_load_providers_failed");
              setToastMessage(message);
              focusComposer();
            });
        },
    ...
    return candidates.slice(0, 80).map((item) => ({
      ...
      meta:
        item.workspaceId === props.selectedWorkspaceId
          ? translate("session.cmd_current_workspace")
          : translate("session.cmd_switch_meta"),
      action: () => {
        closeCommandPalette();
  ...
  const commandPaletteTitle = createMemo(() => {
    const mode = commandPaletteMode();
    if (mode === "sessions") return translate("session.cmd_search_sessions");
    return translate("session.cmd_quick_actions");
  });

  const commandPalettePlaceholder = createMemo(() => {
    const mode = commandPaletteMode();
    if (mode === "sessions") return translate("session.cmd_find_by_title");
    return translate("session.cmd_search_actions");
  });
  ...
  const updatePillLabel = createMemo(() => {
    const state = props.updateStatus?.state;
    if (state === "ready") {
      return props.anyActiveRuns ? translate("session.update_ready") : translate("session.update_install");
    }
    if (state === "downloading") {
      const percent = updateDownloadPercent();
      return percent == null ? translate("session.update_downloading") : `${translate("session.update_downloading")} ${percent}%`;
    }
    return translate("session.update_available");
  });
  ...
  const updatePillTitle = createMemo(() => {
    ...
    if (state === "ready") {
      return props.anyActiveRuns
        ? `${translate("session.update_ready")} ${version}. ${translate("session.update_stop_runs_to_restart")}`
        : `${translate("session.update_restart_to_apply")} ${version}`;
    }
    if (state === "downloading") return `${translate("session.update_downloading_update")} ${version}`;
    return `${translate("session.update_available")} ${version}`;
  });
  ...
  const openProviderAuth = (preferredProviderId?: string) => {
    void props.openProviderAuthModal({ preferredProviderId }).catch((error) => {
      const message = error instanceof Error ? error.message : translate("session.connect_failed");
      setToastMessage(message);
    });
  };
  ...
  return (
    <div class="h-[100dvh] min-h-screen w-full overflow-hidden bg-[var(--dls-app-bg)] p-2 md:p-2.5 text-dls-text font-sans">
      <div class="flex h-full w-full gap-2 md:gap-2.5">
        ...
        <main class="min-w-0 flex-1 flex flex-col overflow-hidden rounded-[24px] border border-dls-border bg-dls-surface shadow-[var(--dls-shell-shadow)]">
          <header class="z-10 flex h-12 shrink-0 items-center justify-between border-b border-dls-border bg-dls-surface px-4 md:px-6">
            <div class="flex min-w-0 items-center gap-3">
              ...
              <h1 class="truncate text-[15px] font-semibold text-dls-text">
                {showWorkspaceSetupEmptyState()
                  ? translate("session.create_or_connect_workspace")
                  : selectedSessionTitle() || DEFAULT_SESSION_TITLE}
              </h1>
              <span class="hidden truncate text-[13px] text-dls-secondary lg:inline">
                {props.selectedWorkspaceDisplay.displayName || props.selectedWorkspaceDisplay.name || translate("session.workspace_fallback")}
              </span>
            ...
            <div class="flex items-center gap-1.5 text-dls-secondary">
              ...
              <button
                ...
                title={translate("session.search_conversation_hint")}
                aria-label={translate("session.search_conversation_label")}
              >
                <Search size={16} />
              </button>
              ...
              <div class="flex min-h-[104px] items-start gap-3 px-4 py-5 sm:px-6 lg:px-10">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <div class="min-w-0">
                      ...
                      <div class="mt-1 space-y-1 text-sm leading-relaxed text-dls-secondary">
                        <div>
                          ...
                          >
                            <span class="font-medium text-amber-11">{translate("session.reload_will_stop_tasks")}</span>
                          </Show>
                        </div>
                        ...
                        <Show when={props.reloadBannerBlocked}>
                          <div class="flex items-start gap-2 rounded-2xl border border-amber-6/40 bg-amber-3/70 px-3 py-2 text-xs text-amber-11">
                            ...
                            <span>
                              {`${translate("session.reload_stops")} ${props.reloadBannerActiveCount} ${translate("session.reload_active_conversations")}${props.reloadBannerActiveCount === 1 ? "" : translate("session.reload_plural_suffix")}.`}
                            </span>
                          </div>
                        </Show>
                      </div>
                  ...
                  <div class="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    >
                      {props.reloadBusy
                        ? translate("session.reloading")
                        : props.reloadBannerBlocked
                          ? translate("session.reload_and_stop_tasks")
                          : translate("session.reload_now")}
                    </Button>
                    <Button
                      variant="ghost"
                      class="rounded-full px-3 py-1.5 text-xs"
                        ...
                        <div class="flex flex-col gap-6">
                          <div class="flex flex-col items-center text-center">
                            ...
                            <p class="mt-3 max-w-xl text-sm leading-6 text-dls-secondary sm:text-[15px]">
                              {translate("session.setup_description")}
                            </p>
                          </div>
                          ...
                          <div class="grid gap-3 sm:grid-cols-[1.2fr_1fr]">
                            ...
                            >
                              <div class="flex items-start gap-4">
                                ...
                                <div class="min-w-0">
                                  <div class="text-base font-semibold">{translate("session.get_started")}</div>
                                  <div class="mt-1 text-sm leading-6 text-white/80">
                                    {translate("session.get_started_description")}
                                  </div>
                                </div>
                              </div>
                            </button>
                            ...
                            >
                              <div class="flex items-start gap-4">
                                ...
                                <div class="min-w-0">
                                  <div class="text-base font-semibold">{translate("session.pick_folder")}</div>
                                  <div class="mt-1 text-sm leading-6 text-dls-secondary">
                                    {translate("session.pick_folder_description")}
                                  </div>
                                </div>
                              </div>
                            </button>
                    ...
                    <div class="mb-4 flex justify-center">
                      ...
                      >
                        {props.loadingEarlierMessages
                          ? translate("session.loading_earlier")
                          : hiddenMessageCount() > 0
                            ? `${translate("session.show_earlier_prefix")} ${nextRevealCount().toLocaleString()} ${translate("session.show_earlier_suffix")}${nextRevealCount() === 1 ? "" : translate("session.plural_s")}`
                            : translate("session.load_earlier")}
                      </button>
                    </div>
                  </Show>
                ...
                <div class="absolute bottom-4 left-0 right-0 z-20 flex justify-center pointer-events-none">
                  <div class="pointer-events-auto flex items-center gap-2 rounded-full border border-dls-border bg-dls-surface/95 p-1 shadow-[var(--dls-card-shadow)] backdrop-blur-md">
                    <Show when={Boolean(topClippedMessageId())}>
                      ...
                      >
                        {translate("session.jump_to_start")}
                      </button>
                    </Show>
          ...
          <StatusBar
            ...
            auroworkServerStatus={props.auroworkServerStatus}
            developerMode={props.developerMode}
            settingsOpen={false}
            ...
            statusLabel={
              showRunIndicator()
                ? translate("session.status_active")
                : props.selectedSessionId
                  ? translate("session.status_ready")
                  : translate("session.ready")
            }
            statusDotClass={
              showRunIndicator()
                ? "bg-green-9"
        ...
        >
          ...
          >
              ...
              <Show
                ...
                fallback={
                  <div class="px-3 py-6 text-sm text-dls-secondary text-center">
                    {translate("session.cmd_no_matches")}
                  </div>
                }
              >
            ...
            <div class="border-t border-dls-border px-3 py-2 text-[11px] text-dls-secondary flex items-center justify-between gap-2">
              <span>{translate("session.cmd_arrow_keys")}</span>
              <span>{translate("session.cmd_enter_esc")}</span>
            </div>
          </div>
        </div>
      </Show>
      ...
      <ConfirmModal
        ...
        title={translate("session.delete_session_title")}
        message={
          sessionTitleForId(deleteSessionId()).trim()
            ? `${translate("session.delete_session_named_prefix")} "${sessionTitleForId(deleteSessionId()).trim()}" ${translate("session.delete_session_named_suffix")}`
            : translate("session.delete_session_message")
        }
        confirmLabel={deleteSessionBusy() ? translate("session.deleting") : translate("session.delete_label")}
        cancelLabel={translate("session.cancel_label")}
        variant="danger"
