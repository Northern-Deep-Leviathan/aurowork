...
function resolveOrchestratorPackageJson() {
  ...
  if (globalRoot.status === 0) {
    const candidate = resolve(globalRoot.stdout.trim(), "aurowork-orchestrator", "package.json");
  ...
  throw new Error("Unable to locate aurowork-orchestrator package.json");
}
...
const downloadUrl = process.env.AUROWORK_OPENCODE_DOWNLOAD_URL?.trim()
  || (version
