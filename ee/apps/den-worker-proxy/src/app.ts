...
import { and, eq, isNull } from "@aurowork-ee/den-db/drizzle"
import { createDenDb, DaytonaSandboxTable, RateLimitTable, WorkerTokenTable } from "@aurowork-ee/den-db"
import { createDenTypeId, normalizeDenTypeId } from "@aurowork-ee/utils/typeid"
...
const publicCorsAllowHeaders = [
  "Authorization",
  "Content-Type",
  "X-AuroWork-Host-Token",
  "X-AuroWork-Client-Id",
...
async function resolveWorkerTokenScope(workerId: WorkerId, request: Request): Promise<WorkerTokenScope | "invalid" | null> {
  const hostToken = request.headers.get("x-aurowork-host-token")?.trim() || null
...
app.all("*", async (c) => {
  ...
  if (requestUrl.pathname === "/") {
    return Response.redirect("https://auroworklabs.com", 302)
  }
