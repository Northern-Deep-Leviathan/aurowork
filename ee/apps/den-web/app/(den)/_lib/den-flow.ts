...
export const LAST_WORKER_STORAGE_KEY = "aurowork:web:last-worker";
export const PENDING_SOCIAL_SIGNUP_STORAGE_KEY = "aurowork:web:pending-social-signup";
export const AUTH_TOKEN_STORAGE_KEY = "aurowork:web:auth-token";
export const ONBOARDING_INTENT_STORAGE_KEY = "aurowork:web:onboarding-intent";
export const WORKER_STATUS_POLL_MS = DEN_WORKER_POLL_INTERVAL_MS;
export const DEFAULT_AUTH_NAME = "AuroWork User";
export const DEFAULT_WORKER_NAME = "My Worker";
export const AUROWORK_APP_CONNECT_BASE_URL = (process.env.NEXT_PUBLIC_AUROWORK_APP_CONNECT_URL ?? "").trim();
export const AUROWORK_AUTH_CALLBACK_BASE_URL = (process.env.NEXT_PUBLIC_AUROWORK_AUTH_CALLBACK_URL ?? "").trim();
...
export function getSocialCallbackUrl(): string {
  try {
    const origin = typeof window !== "undefined" ? window.location.origin : AUROWORK_AUTH_CALLBACK_BASE_URL || "https://app.auroworklabs.com";
    ...
    return "https://app.auroworklabs.com/";
  }
}
...
export function getWorker(payload: unknown): WorkerLaunch | null {
  ...
  return {
    ...
    auroworkUrl: instance && typeof instance.url === "string" ? instance.url : null,
    workspaceId: null,
...
export function getWorkerTokens(payload: unknown): WorkerTokens | null {
  ...
  const auroworkUrl = connect && typeof connect.auroworkUrl === "string" ? connect.auroworkUrl : null;
  ...
  return { clientToken, ownerToken, hostToken, auroworkUrl, workspaceId };
}
...
export function isWorkerLaunch(value: unknown): value is WorkerLaunch {
  ...
  return (
    ...
    (typeof value.auroworkUrl === "string" || value.auroworkUrl === null || typeof value.auroworkUrl === "undefined") &&
...
export function buildAuroworkDeepLink(
  auroworkUrl: string | null,
  accessToken: string | null,
  workerId: string | null,
  workerName: string | null
): string | null {
  if (!auroworkUrl || !accessToken) {
    return null;
  }

  const params = new URLSearchParams({
    auroworkHostUrl: auroworkUrl,
    auroworkToken: accessToken,
    source: "aurowork-web"
  });
  ...
  return `aurowork://connect-remote?${params.toString()}`;
}

export function buildAuroworkAppConnectUrl(
  ...
  if (!appConnectBaseUrl || !auroworkUrl || !accessToken) {
    return null;
  }
  ...
  connectUrl.searchParams.set("auroworkHostUrl", auroworkUrl);
  connectUrl.searchParams.set("auroworkToken", accessToken);
  if (options?.autoConnect) {
    connectUrl.searchParams.set("autoConnect", "1");
  }
  connectUrl.searchParams.set("source", "aurowork-web");
...
export async function resolveAuroworkWorkspaceUrl(instanceUrl: string, accessToken: string): Promise<{ workspaceId: string; auroworkUrl: string } | null> {
  ...
  return {
    workspaceId,
    auroworkUrl: buildWorkspaceUrl(baseUrl, workspaceId)
  };
}
