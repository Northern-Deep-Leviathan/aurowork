...
import {
  ...
  buildAuroworkAppConnectUrl,
  buildAuroworkDeepLink,
...
type DenFlowContextValue = {
  ...
  events: LaunchEvent[];
  auroworkDeepLink: string | null;
  auroworkAppConnectUrl: string | null;
...
export function DenFlowProvider({ children }: { children: ReactNode }) {
  ...
  const [desktopAuthScheme, setDesktopAuthScheme] = useState("aurowork");
  ...
  const activeWorker =
    worker && workerLookupId === worker.workerId
      ...
      : selectedWorker
        ...
        : worker;
  const auroworkConnectUrl = activeWorker?.auroworkUrl ?? activeWorker?.instanceUrl ?? null;
  const preferredAuroworkToken = activeWorker?.ownerToken ?? activeWorker?.clientToken ?? null;
  const hasWorkspaceScopedUrl = Boolean(auroworkConnectUrl && /\/w\/[^/?#]+/.test(auroworkConnectUrl));
  const auroworkDeepLink = buildAuroworkDeepLink(
    auroworkConnectUrl,
    preferredAuroworkToken,
    activeWorker?.workerId ?? null,
    activeWorker?.workerName ?? null
  );
  const auroworkAppConnectUrl = buildAuroworkAppConnectUrl(
    AUROWORK_APP_CONNECT_BASE_URL,
    auroworkConnectUrl,
    preferredAuroworkToken,
    activeWorker?.workerId ?? null,
  ...
  async function finalizeEmailPasswordSignIn(
    ...
    if (desktopAuthRequested) {
      setAuthInfo("Signed in. Returning to AuroWork...");
      return null;
    }
  ...
  async function withResolvedAuroworkCredentials(candidate: WorkerLaunch, options: { quiet?: boolean } = {}) {
    const existingConnectUrl = candidate.auroworkUrl?.trim() ?? "";
    const existingWorkspaceId = candidate.workspaceId?.trim() ?? "";
    if (existingConnectUrl && existingWorkspaceId) {
      return {
        ...candidate,
        auroworkUrl: existingConnectUrl,
    ...
    if (!accessToken) {
      ...
      return {
        ...candidate,
        auroworkUrl: instanceUrl.trim().replace(/\/+$/, ""),
        workspaceId: mountedWorkspaceId
      };
    }

    try {
      const resolved = await resolveAuroworkWorkspaceUrl(instanceUrl, accessToken);
      if (resolved) {
        return {
          ...candidate,
          auroworkUrl: resolved.auroworkUrl,
    ...
    try {
      const { response, payload } = await requestJson(
        `/v1/workers/${encodeURIComponent(targetWorkerId)}/runtime/upgrade`,
        {
          method: "POST",
          headers: authToken ? { Authorization: `Bearer ${authToken}` } : undefined,
          body: JSON.stringify({ services: ["aurowork-server", "opencode"] })
        },
        12000
      );
  ...
  async function completeDesktopAuthHandoff() {
    ...
    try {
      ...
      const auroworkPayload = payload as { auroworkUrl?: unknown } | null;
      const auroworkUrl = typeof auroworkPayload?.auroworkUrl === "string" ? auroworkPayload.auroworkUrl.trim() : "";
      if (!auroworkUrl) {
        setAuthError("Desktop handoff succeeded, but no AuroWork redirect URL was returned.");
        return;
      }

      setDesktopRedirectUrl(auroworkUrl);
      window.location.assign(auroworkUrl);
    } catch (error) {
      setAuthError(error instanceof Error ? error.message : "Failed to open AuroWork.");
    } finally {
  ...
  async function launchWorker(options: { source?: "manual" | "signup_auto"; workerNameOverride?: string } = {}) {
    ...
    try {
      ...
      const resolvedWorker = await withResolvedAuroworkCredentials(parsedWorker);
  ...
  async function checkWorkerStatus(options: { workerId?: string; quiet?: boolean; background?: boolean } = {}) {
    ...
    try {
      ...
      const nextWorker: WorkerLaunch =
        worker && worker.workerId === summary.workerId
          ...
          : {
              ...
              provider: summary.provider,
              instanceUrl: summary.instanceUrl,
              auroworkUrl: summary.instanceUrl,
              workspaceId: null,
              clientToken: null,
              ownerToken: null,
              hostToken: null
      ...
      const resolvedWorker = await withResolvedAuroworkCredentials(nextWorker, { quiet: true });
      setWorker(resolvedWorker);
      setPendingRestoredWorkerId(null);
    ...
    try {
      ...
      const nextWorker: WorkerLaunch =
        worker && worker.workerId === id
          ? {
              ...worker,
              auroworkUrl: tokens.auroworkUrl ?? worker.auroworkUrl,
          ...
          : {
              ...
              provider: null,
              instanceUrl: null,
              auroworkUrl: tokens.auroworkUrl,
  ...
  useEffect(() => {
    ...
    try {
      ...
      const restored: WorkerLaunch = {
        ...parsed,
        auroworkUrl: parsed.auroworkUrl ?? parsed.instanceUrl,
