...
export function DashboardScreen({ showSidebar = true }: { showSidebar?: boolean }) {
  ...
  const webDisabled = !auroworkAppConnectUrl || !isReady;
  const desktopDisabled = !auroworkDeepLink || !isReady;
  const showConnectionHint = !auroworkDeepLink || !hasWorkspaceScopedUrl;
                ...
                <div className="flex max-w-xl flex-col gap-6">
                  ...
                  <p className="text-[16px] leading-relaxed text-[var(--dls-text-secondary)] md:text-[17px]">
                    {isReady
                      ...
                      : "We are allocating resources and preparing the AuroWork connection before unlocking the rest of the controls."}
                  </p>
                ...
                <div className="flex w-full flex-col gap-3 md:w-[200px] md:shrink-0 md:items-end">
                  ...
                  <div className="hidden w-full flex-col items-center md:flex">
                    <button
                      ...
                      onClick={() => {
                        if (!desktopDisabled && auroworkDeepLink) {
                          window.location.href = auroworkDeepLink;
                        }
                      }}
                    ...
                    <span className="mt-2 text-[11px] font-medium text-[var(--dls-text-secondary)]">requires the AuroWork desktop app</span>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="px-8 pb-8 pt-0">
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        disabled={!auroworkDeepLink || !isReady}
                      >
                        {auroworkDeepLink ? "Open in Desktop" : "Preparing connection..."}
                      </button>
                      <button
                        type="button"
                    ...
                    <div className="mt-6 space-y-4">
                      <CredentialRow
                        ...
                        value={activeWorker?.auroworkUrl ?? activeWorker?.instanceUrl ?? null}
                        placeholder="Connection URL is still preparing..."
                        hint={showConnectionHint ? (!auroworkDeepLink ? "Getting connection details ready..." : "Finishing your workspace URL...") : undefined}
                        canCopy={Boolean(activeWorker?.auroworkUrl ?? activeWorker?.instanceUrl)}
                        copied={copiedField === "aurowork-url"}
                        onCopy={() => void copyToClipboard("aurowork-url", activeWorker?.auroworkUrl ?? activeWorker?.instanceUrl ?? null)}
                        muted={!isReady}
                      />
          ...
          <div className="flex flex-col gap-4 md:gap-6">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-lg font-semibold tracking-tight text-[var(--dls-text-primary)]">AuroWork</span>
              </div>
              <button
                type="button"
