...
const schema = z.object({
  ...
  AUROWORK_DEV_MODE: z.string().optional(),
  ...
  AUROWORK_DAYTONA_ENV_PATH: z.string().optional(),
  ...
  RENDER_WORKER_AUROWORK_VERSION: z.string().optional(),
  ...
  DAYTONA_AUROWORK_PORT: z.string().optional(),
...
export const env = {
  ...
  devMode: (parsed.AUROWORK_DEV_MODE ?? "0").trim() === "1",
  github: {
  ...
  render: {
    ...
    workerAuroworkVersion: parsed.RENDER_WORKER_AUROWORK_VERSION,
  ...
  daytona: {
    envPath: optionalString(parsed.AUROWORK_DAYTONA_ENV_PATH),
    ...
    workerProxyBaseUrl:
      optionalString(parsed.DAYTONA_WORKER_PROXY_BASE_URL) ?? "https://workers.den.auroworklabs",
    sandboxNamePrefix:
    ...
    dataMountPath:
      optionalString(parsed.DAYTONA_DATA_MOUNT_PATH) ?? "/persist/aurowork",
    runtimeWorkspacePath:
      optionalString(parsed.DAYTONA_RUNTIME_WORKSPACE_PATH) ??
      "/tmp/aurowork-workspace",
    runtimeDataPath:
      optionalString(parsed.DAYTONA_RUNTIME_DATA_PATH) ?? "/tmp/aurowork-data",
    sidecarDir:
      optionalString(parsed.DAYTONA_SIDECAR_DIR) ?? "/tmp/aurowork-sidecars",
    auroworkPort: Number(parsed.DAYTONA_AUROWORK_PORT ?? "8787"),
