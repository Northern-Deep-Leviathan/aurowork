...
async function provisionWorkerOnRender(
  ...
  const startCommand = [
    ...
    "attempt=0; while [ $attempt -lt 3 ]; do attempt=$((attempt + 1)); aurowork serve --workspace /tmp/workspace --remote-access --aurowork-port ${PORT:-10000} --opencode-host 127.0.0.1 --opencode-port 4096 --connect-host 127.0.0.1 --cors '*' --approval manual --allow-external --opencode-source external --opencode-bin ./bin/opencode --verbose && exit 0; echo \"aurowork serve failed (attempt $attempt); retrying in 3s\"; sleep 3; done; exit 1",
  ].join(" && ");
