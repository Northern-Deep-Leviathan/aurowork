...
function sandboxLabels(workerId: WorkerId) {
  return {
    "aurowork.den.provider": "daytona",
    "aurowork.den.worker-id": workerId,
  }
}
...
function buildAuroWorkStartCommand(input: ProvisionInput) {
  const verifyRuntimeStep = [
    "if ! command -v aurowork >/dev/null 2>&1; then echo 'aurowork binary missing from Daytona runtime image; rebuild and republish the Daytona snapshot' >&2; exit 1; fi",
  ...
  const auroworkServe = [
    ...
    ` --remote-access`,
    ` --aurowork-port ${env.daytona.auroworkPort}`,
...
while [ "$attempt" -lt 3 ]; do
  attempt=$((attempt + 1))
  if ${auroworkServe}; then
    exit 0
  fi
  status=$?
  echo "aurowork serve failed (attempt $attempt, exit $status); retrying in 3s"
  sleep 3
done
exit 1
`.trim()
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    try {
      ...
      if (typeof command.exitCode === "number" && command.exitCode !== 0) {
        ...
        throw new Error(
          [
            `aurowork session exited with ${command.exitCode}`,
      ...
      if (error instanceof Error && error.message.startsWith("aurowork session exited")) {
        throw error
      }
    }
...
export async function refreshDaytonaSignedPreview(workerId: WorkerId) {
  ...
  const preview = await sandbox.getSignedPreviewUrl(env.daytona.auroworkPort, expiresInSeconds)
...
export async function provisionWorkerOnDaytona(
  ...
  try {
    ...
    const sessionId = `aurowork-${workerHint(input.workerId)}`
    await sandbox.process.createSession(sessionId)
    const command = await sandbox.process.executeSessionCommand(
      sessionId,
      {
        command: buildAuroWorkStartCommand(input),
        runAsync: true,
      },
      0,
    )
