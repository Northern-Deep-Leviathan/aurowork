...
function parseWorkspaceSelection(payload: unknown): { workspaceId: string; auroworkUrl: string } | null {
  ...
  return {
    workspaceId,
    auroworkUrl: `${baseUrl}/w/${encodeURIComponent(workspaceId)}`,
  }
}
    ...
    try {
      const response = await fetch(`${normalizeUrl(candidate)}${input.path}`, {
        ...
        headers: {
          ...
          "X-AuroWork-Host-Token": access.hostToken,
        },
...
workersRouter.post("/", asyncRoute(async (req, res) => {
  ...
  if (parsed.data.destination === "cloud" && !env.devMode && (await countUserCloudWorkers(session.user.id)) > 0) {
    const access = await requireCloudWorkerAccess({
      userId: session.user.id,
      email: session.user.email ?? `${session.user.id}@placeholder.local`,
      name: session.user.name ?? session.user.email ?? "AuroWork User",
    })
    if (!access.allowed) {
      res.status(402).json({
...
workersRouter.post("/:id/tokens", asyncRoute(async (req, res) => {
  ...
  res.json({
    ...
    connect: connect ?? (instance?.url ? { auroworkUrl: instance.url, workspaceId: null } : null),
  })
}))
