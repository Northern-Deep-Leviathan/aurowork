...
function isWebAppHost(hostname: string): boolean {
  ...
  return normalized === "app.auroworklabs.com"
    || normalized === "app.aurowork.software"
...
function resolveDesktopDenBaseUrl(req: express.Request): string {
  ...
  if (!targetHost) {
    return "https://app.auroworklabs.com/api/den"
  }
...
function buildAuroworkDeepLink(input: {
  scheme?: string | null
  grant: string
  denBaseUrl: string
}) {
  const requestedScheme = input.scheme?.trim() || "aurowork"
...
desktopAuthRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    res.json({
      ...
      auroworkUrl: buildAuroworkDeepLink({
        scheme: parsed.data.desktopScheme || "aurowork",
        grant,
        denBaseUrl,
      }),
    })
...
desktopAuthRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const rows = await db
      ...
      .innerJoin(
        AuthSessionTable,
        eq(DesktopHandoffGrantTable.session_token, AuthSessionTable.token),
      )
      .innerJoin(
        AuthUserTable,
        eq(DesktopHandoffGrantTable.user_id, AuthUserTable.id),
      )
      .where(
        and(
