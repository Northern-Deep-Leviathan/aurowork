...
const daytonaEnvPath = process.env.AUROWORK_DAYTONA_ENV_PATH?.trim() || findUpwards(repoRoot, ".env.daytona")
if (daytonaEnvPath) {
...
async function main() {
  ...
  const containerName = existingBaseUrl
    ? null
    : `aurowork-den-daytona-${randomUUID().slice(0, 8)}`
  const dbName = "aurowork_den_daytona_e2e"
  const dbPassword = "aurowork-root"
  ...
  const runtimeEnv = {
    ...
    BETTER_AUTH_SECRET: "aurowork-den-daytona-secret-0000000000",
    BETTER_AUTH_URL: baseUrl,
    ...(appPort ? { PORT: String(appPort) } : {}),
    CORS_ORIGINS: baseUrl,
    PROVISIONER_MODE: "daytona",
    POLAR_FEATURE_GATE_ENABLED: "false",
    AUROWORK_DAYTONA_ENV_PATH: daytonaEnvPath || process.env.AUROWORK_DAYTONA_ENV_PATH || "",
  }
  ...
  try {
    ...
    log("Checking AuroWork connect metadata...")
    const tokensResponse = await requestJson(baseUrl, `/v1/workers/${workerId}/tokens`, {
      method: "POST",
      ...auth,
    })
    if (!tokensResponse.response.ok || !tokensResponse.payload?.connect?.auroworkUrl) {
    ...
    const statusResponse = await fetch(`${tokensResponse.payload.connect.auroworkUrl}/status`, {
    ...
    const capabilitiesResponse = await fetch(`${tokensResponse.payload.connect.auroworkUrl}/capabilities`, {
      headers: connectHeaders,
    })
