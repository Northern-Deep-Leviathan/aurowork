...
- the AuroWork Cloud web app on a randomized host port
...
- `AUROWORK_DAYTONA_ENV_PATH` optional path to a shared `.env.daytona` file; when unset, Den searches upwards from the repo for `.env.daytona`
...
- `RENDER_WORKER_AUROWORK_VERSION` `aurowork-orchestrator` npm version installed in workers; the worker build reads the pinned OpenCode version from `constants.json` shipped with that package and bundles the matching `opencode` binary into the Render deploy
- `RENDER_WORKER_NAME_PREFIX` service name prefix
- `RENDER_WORKER_PUBLIC_DOMAIN_SUFFIX` optional domain suffix for worker custom URLs (e.g. `aurowork.studio` -> `<worker-id>.aurowork.studio`)
...
- `VERCEL_DNS_DOMAIN` Vercel-managed DNS zone used for worker records (default `aurowork.studio`)
...
- Daytona:
  ...
  - `DAYTONA_SIGNED_PREVIEW_EXPIRES_SECONDS` TTL for the signed AuroWork preview URL returned to Den clients (Daytona currently caps this at 24 hours)
  ...
  - `DAYTONA_RUNTIME_WORKSPACE_PATH`, `DAYTONA_RUNTIME_DATA_PATH`, `DAYTONA_SIDECAR_DIR` local sandbox paths used for the live AuroWork runtime; the mounted Daytona volumes are linked into the runtime workspace under `volumes/`
  - `DAYTONA_AUROWORK_PORT`, `DAYTONA_OPENCODE_PORT` ports used when launching `aurowork serve`
...
For local Daytona development, place your Daytona API credentials in `/_repos/aurowork/.env.daytona` and Den will pick them up automatically, including from task worktrees.

In local dev (`AUROWORK_DEV_MODE=1`), Den prints email verification codes to the server logs instead of sending them through Loops.
...
- `scripts/create-daytona-aurowork-snapshot.sh`
...
```bash
./scripts/create-daytona-aurowork-snapshot.sh aurowork-runtime
```
...
If you do not set `DAYTONA_SNAPSHOT`, Den falls back to `DAYTONA_SANDBOX_IMAGE`. That image must already include `aurowork` and `opencode` on `PATH`.
...
- Snapshot naming: `snapshot_name` input if provided, otherwise `${DAYTONA_SNAPSHOT_NAME_BASE:-aurowork}-<tag-without-v>`
...
- `DEN_RENDER_WORKER_AUROWORK_VERSION` pins the `aurowork-orchestrator` npm version installed in workers; the worker build bundles the matching `opencode` release asset into the Render image
- `DEN_CORS_ORIGINS` (defaults to `https://app.auroworklabs.com,https://api.auroworklabs.com,<render-service-url>`)
- `DEN_BETTER_AUTH_TRUSTED_ORIGINS` (defaults to `DEN_CORS_ORIGINS`)
- `DEN_RENDER_WORKER_PUBLIC_DOMAIN_SUFFIX` (defaults to `aurowork.studio`)
- `DEN_RENDER_CUSTOM_DOMAIN_READY_TIMEOUT_MS` (defaults to `240000`)
- `DEN_BETTER_AUTH_URL` (defaults to `https://app.auroworklabs.com`)
...
- `DEN_VERCEL_DNS_DOMAIN` (defaults to `aurowork.studio`)
- `DEN_POLAR_FEATURE_GATE_ENABLED` (`true`/`false`, defaults to `false`)
- `DEN_POLAR_API_BASE` (defaults to `https://api.polar.sh`)
- `DEN_POLAR_SUCCESS_URL` (defaults to `https://app.auroworklabs.com`)
