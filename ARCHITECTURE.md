...
We move most of the functionality to the aurowork server which interfaces mostly with FS and proxies to the auro engine.
...
- `/apps/orchestrator/`: AuroWork orchestrator CLI/daemon. In `start`/`serve` host mode it manages AuroWork server + the auro engine + `opencode-router`; in daemon mode it manages workspace activation and auro engine lifecycle for desktop runtime.
...
- The auro engine (upstream OpenCode binary) runs on loopback (default `127.0.0.1:4096`).
- The AuroWork server also defaults to loopback-only access. Remote sharing is an explicit opt-in that rebinds the AuroWork server to `0.0.0.0` while keeping the auro engine on loopback.
...
This model keeps the user experience consistent across self-hosted and hosted paths while preserving auro engine parity.
...
- `/apps/desktop/` exposes native commands (`engine_*`, `orchestrator_*`, `aurowork_server_*`, `opencodeRouter_*`) to start/stop local services and report status to the UI.
- Runtime selection in desktop:
  - `aurowork-orchestrator` (default): Tauri launches `aurowork daemon run` and uses it for workspace activation plus auro engine lifecycle.
  - `direct`: Tauri starts the auro engine directly.
- In both desktop runtimes, AuroWork server (`/apps/server/`) is the API surface consumed by the UI; it is started with the resolved auro engine base URL and proxies the auro engine and `opencode-router` routes.
- Desktop-launched auro engine credentials are always random, per-launch values generated by AuroWork. The auro engine stays on loopback and is intended to be reached through AuroWork server rather than exposed directly.
...
/ee/apps/den-controller
    ...
    |        |
    |        v
    |      /ee/apps/den-worker-runtime -> aurowork serve + auro engine
    |
...
AuroWork can expose a workspace through Slack and Telegram via `opencode-router`.
...
AuroWork optimizes for a predictable routing boundary.
...
AuroWork server proxies `/opencode-router/*` and `/w/:id/opencode-router/*` to that local router API.

### Workspace-scoped behavior in AuroWork

AuroWork server treats messaging identities as workspace-scoped.
...
workspace A ----\
workspace B -----+--> AuroWork server knows all workspaces
workspace C ----/
...
- AuroWork server is already multi-workspace aware.
...
Desktop local AuroWork server ports:

- Desktop-hosted local AuroWork server instances do not assume a fixed `8787` port.
