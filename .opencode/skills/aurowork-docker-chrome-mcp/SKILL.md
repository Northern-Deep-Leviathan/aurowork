---
name: aurowork-docker-chrome-mcp
description: |
  Start the AuroWork dev stack via Docker and verify real user flows via Chrome MCP.
...
Run from the AuroWork repo root:
```bash
...
- AuroWork server URL (http://localhost:<AUROWORK_PORT>)
- Token file path (`tmp/.dev-env-<id>`) containing `AUROWORK_TOKEN` + `AUROWORK_HOST_TOKEN`
...
```bash
docker ps --format '{{.Names}}' | rg '^aurowork-dev-'
```
