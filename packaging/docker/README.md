...
What it does:
- Starts **headless** (OpenCode + AuroWork server) on port 8787
...
If you want to seed the container from your host OpenCode state for debugging, run with `AUROWORK_DOCKER_DEV_MOUNT_HOST_OPENCODE=1`. This imports host config/auth into the isolated dev state instead of mounting live host state directly.
...
- Health check: `curl http://localhost:<aurowork_port>/health`

Optional env vars (via `.env` or `export`):
- `AUROWORK_TOKEN` — fixed client token
- `AUROWORK_HOST_TOKEN` — fixed host/admin token
- `AUROWORK_WORKSPACE` — host path to mount as workspace
- `AUROWORK_PORT` — host port to map to container :8787
- `WEB_PORT` — host port to map to container :5173
- `SHARE_PORT` — host port to map to the local share service :3000
- `AUROWORK_PUBLIC_HOST` — host name/IP used in printed LAN/public URLs (defaults to your machine hostname)
- `AUROWORK_DOCKER_DEV_MOUNT_HOST_OPENCODE=1` — import host OpenCode config/auth into the isolated dev state
- `AUROWORK_OPENCODE_CONFIG_DIR` — override the host OpenCode config source used for that optional import
- `AUROWORK_OPENCODE_DATA_DIR` — override the host OpenCode data source used for that optional import

The dev stack also starts the local share service automatically and points the AuroWork app at it, so share-link flows publish to a local service instead of `https://share.auroworklabs.com`.
...
- Starts the **AuroWork Cloud web app** on port 3005 inside Docker
...
This is a minimal packaging template to run the AuroWork Host contract in a single container.

It runs:

- `opencode serve` (engine) bound to `127.0.0.1:4096` inside the container
- `aurowork-server` published on `0.0.0.0:8787` via an explicit `--remote-access` launch path (the only published surface)
...
- `AUROWORK_TOKEN` (client token)
- `AUROWORK_HOST_TOKEN` (host/owner token)

Optional:

- `AUROWORK_APPROVAL_MODE=auto|manual`
- `AUROWORK_APPROVAL_TIMEOUT_MS=30000`

Persistence:

- Workspace is mounted at `/workspace`
- Host data dir is mounted at `/data` (OpenCode caches + AuroWork server config/tokens)

### Notes

- OpenCode is not exposed directly; access it via the AuroWork proxy (`/opencode/*`).
