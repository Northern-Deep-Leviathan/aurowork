...
#
# Usage (from _repos/aurowork repo root):
#   packaging/docker/dev-up.sh
#
# Defaults to isolated OpenCode dev state inside the container.
# Escape hatch: set AUROWORK_DOCKER_DEV_MOUNT_HOST_OPENCODE=1 to import host
...
MOUNT_HOST_OPENCODE="${AUROWORK_DOCKER_DEV_MOUNT_HOST_OPENCODE:-0}"

resolve_opencode_config_dir() {
  local override="${AUROWORK_OPENCODE_CONFIG_DIR:-}"
  if [ -n "$override" ]; then
    if [ -d "$override" ]; then
      printf '%s\n' "$override"
      return 0
    fi
    echo "warning: AUROWORK_OPENCODE_CONFIG_DIR is not a directory: $override" >&2
  fi
...
resolve_opencode_data_dir() {
  local override="${AUROWORK_OPENCODE_DATA_DIR:-}"
  if [ -n "$override" ]; then
    if [ -d "$override" ]; then
      printf '%s\n' "$override"
      return 0
    fi
    echo "warning: AUROWORK_OPENCODE_DATA_DIR is not a directory: $override" >&2
  fi
...
detect_public_host() {
  if [ -n "${AUROWORK_PUBLIC_HOST:-}" ]; then
    printf '%s\n' "$AUROWORK_PUBLIC_HOST"
    return
  fi
...
AUROWORK_PORT="$(pick_port)"
WEB_PORT="$(pick_port)"
if [ "$WEB_PORT" = "$AUROWORK_PORT" ]; then
  WEB_PORT="$(pick_port)"
fi
SHARE_PORT="${SHARE_PORT:-$(pick_port)}"
if [ "$SHARE_PORT" = "$AUROWORK_PORT" ] || [ "$SHARE_PORT" = "$WEB_PORT" ]; then
...
echo "- AUROWORK_PORT=$AUROWORK_PORT" >&2
echo "- WEB_PORT=$WEB_PORT" >&2
echo "- SHARE_PORT=$SHARE_PORT" >&2
echo "- AUROWORK_DEV_MODE=1" >&2
if [ "$MOUNT_HOST_OPENCODE" = "1" ]; then
...
start_stack() {
  local config_dir="$1"
  local data_dir="$2"
  AUROWORK_DEV_ID="$DEV_ID" AUROWORK_PORT="$AUROWORK_PORT" WEB_PORT="$WEB_PORT" SHARE_PORT="$SHARE_PORT" \
    AUROWORK_DEV_MODE="1" \
    AUROWORK_PUBLIC_HOST="$PUBLIC_HOST" \
    AUROWORK_HOST_OPENCODE_CONFIG_DIR="$config_dir" \
    AUROWORK_HOST_OPENCODE_DATA_DIR="$data_dir" \
...
echo "- AUROWORK_HOST_OPENCODE_CONFIG_DIR=$ACTIVE_OPENCODE_CONFIG_DIR" >&2
echo "- AUROWORK_HOST_OPENCODE_DATA_DIR=$ACTIVE_OPENCODE_DATA_DIR" >&2
...
echo "" >&2
echo "AuroWork web UI:     http://localhost:$WEB_PORT" >&2
echo "AuroWork web UI (LAN/public): http://$PUBLIC_HOST:$WEB_PORT" >&2
if [ -n "$LAN_IPV4" ]; then
  echo "AuroWork web UI (LAN IP):     http://$LAN_IPV4:$WEB_PORT" >&2
fi
if [ -n "$TAILSCALE_DNS_NAME" ]; then
  echo "AuroWork web UI (Tailscale):  http://$TAILSCALE_DNS_NAME:$WEB_PORT" >&2
fi
echo "AuroWork server:     http://localhost:$AUROWORK_PORT" >&2
echo "AuroWork server (LAN/public): http://$PUBLIC_HOST:$AUROWORK_PORT" >&2
if [ -n "$LAN_IPV4" ]; then
  echo "AuroWork server (LAN IP):     http://$LAN_IPV4:$AUROWORK_PORT" >&2
fi
if [ -n "$TAILSCALE_DNS_NAME" ]; then
  echo "AuroWork server (Tailscale):  http://$TAILSCALE_DNS_NAME:$AUROWORK_PORT" >&2
fi
