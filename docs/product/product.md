...
> Bob the IT guy.
Bob might already use opencode, he can setup agents and workflows and share them with his team. The only thing he needs is a way to share this. The way he does is by using AuroWork and creating "workpaces".
...
Aurowork should be given to give her a good taste of what she can do. 
...
AuroWork provides a first-class plan UI:
...
- The plan is represented in OpenCode as structured `parts` (or a dedicated "plan message") and mirrored in AuroWork.
...
AuroWork lists artifacts per run and supports open/share/download.
...
- Sign in (or sign up) on AuroWork cloud control surface.
...
- Connect from AuroWork app via `Add a worker` -> `Connect remote`.
...
1. User types goal.
2. AuroWork generates plan (structured).
3. User approves.
...
2. AuroWork injects context into session.
3. Execute prompt.
...
1. User opens AuroWork cloud control page and authenticates.
...
5. User clicks `Open in AuroWork` or copies manual credentials.
6. In AuroWork app, user uses `Add a worker` -> `Connect remote` and starts working.
