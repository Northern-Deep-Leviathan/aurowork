# AuroWork Design Language

This is the definitive visual language for AuroWork product and marketing work.
...
*   A major settings panel should use `bg-dls-surface` or `bg-gray-1/40` with a subtle `border-dls-border` (see `_repos/aurowork/apps/app/src/app/pages/settings.tsx`).
...
*   Use flat, unadorned rectangles for tabs (see left rail in `_repos/aurowork/apps/app/src/app/pages/settings.tsx`).
