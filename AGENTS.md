...
AuroWork helps users run agents, skills, and MCP. It is an open-source alternative to Claude Cowork/Codex as a desktop app.

## What AuroWork Is

AuroWork is a practical control surface for agentic work:

* Run local and remote agent workflows from one place.
* Use OpenCode capabilities directly through AuroWork.
* Compose desktop app, server, and messaging connectors without lock-in.
* Treat the AuroWork app as a client of the AuroWork server API surface.
* Connect to hosted workers through a simple user flow: `Add a worker` -> `Connect remote`.

## Core Philosophy

* **Local-first, cloud-ready**: AuroWork runs on your machine in one click and can connect to cloud workflows when needed.
* **Server-consumption first**: the app should consume AuroWork server surfaces (self-hosted or hosted), not invent parallel behavior.
* **Composable**: use the desktop app, WhatsApp/Slack/Telegram connectors, or server mode based on the task.
* **Ejectable**: AuroWork is powered by OpenCode, so anything OpenCode can do is available in AuroWork, even before a dedicated UI exists.
...
AuroWork now has three production-grade ways to run the same product surface:

1. **Desktop-hosted app/server**
   - AuroWork app runs locally and can host server functionality on-device.
2. **CLI-hosted server (aurowork-orchestrator)**
   - AuroWork server surfaces can be provided by the orchestrator/CLI on a trusted machine.
3. **Hosted AuroWork Cloud server**
   - AuroWork-hosted infrastructure provisions workers and exposes the same remote-connect semantics.
...
* **Web parity**: anything that mutates `.opencode/` should be expressible via the AuroWork server API; Tauri-only filesystem calls are a fallback for host mode, not a separate capability set.
...
4. Start the AuroWork dev stack via Docker (from the AuroWork repo root): `packaging/docker/dev-up.sh`.
5. Use Chrome MCP to fully test the feature: `.opencode/skills/aurowork-docker-chrome-mcp/SKILL.md`.
...
AuroWork aims to be a **living system**: agents, skills, commands, and config are hot-reloadable while sessions are running. This enables agents to create new skills or update their own configuration and have changes take effect immediately, without tearing down active sessions.

Design principles for hot reload:

* **Conservative triggers**: only reload when a file that OpenCode reads at startup actually changes inside `.opencode/` or `opencode.json`. Ignore metadata files like `aurowork.json`, `.DS_Store`, etc.
...
* **Auto-reload setting**: each workspace can opt into automatic reload via `.opencode/aurowork.json` (`reload.auto`). When enabled, the engine reloads automatically once queued signals are ready and no sessions are active.
...
* If you change `apps/server/src`, rebuild the AuroWork server binary (`pnpm --filter aurowork-server build:bin`) because `aurowork` (aurowork-orchestrator) runs the compiled server, not the TS sources.
...
This captures AuroWork’s preferred reactivity + UI state patterns (avoid global `busy()` deadlocks; use scoped async state).

## Skill: Trigger a Release

AuroWork releases are built by GitHub Actions (`Release App`). A release is triggered by pushing a `v*` tag (e.g. `v0.1.6`).
`Release App` can also publish aurowork-orchestrator sidecars and npm packages when enabled via workflow inputs or repo vars (`RELEASE_PUBLISH_SIDECARS`, `RELEASE_PUBLISH_NPM`).
...
* `apps/orchestrator/package.json` (`version`, publishes as `aurowork-orchestrator`)
...
## Skill: Publish aurowork-orchestrator (npm)

This is usually covered by `Release App` when `publish_sidecars` + `publish_npm` are enabled. Use `.opencode/skills/aurowork-orchestrator-npm-publish/SKILL.md` for manual recovery or one-off publishing.
